...
- **点検フレーズは点検トリガー**：末尾の点検フレーズや`prh-style.yml`の検出ルールは注意喚起であり、機械置換のターゲットではない
...
比喩を入れるときは、初出時に対応関係を1文で書き、読者が対応関係を頭の中で組み立てなくて済む形にします。比喩で言い換えただけで前進していない段落（次段の説明と接続しないもの）は削ります。書き直しで比喩を消す前に、その比喩が3条件を満たして本文の流れを担っていないか、必ず確認します。3条件を満たす比喩を機械的に削ると、対比構造や読者の足場まで失われることがあります。
