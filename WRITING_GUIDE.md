...
- **方針の文書として読む**：機械置換のための指示集ではなく、書き直しのときに「どこへ着地させたいか」を共有するための文書である
- **書き直しの単位は段落または節**：語句単位の置換は段落の流れを壊しやすいため、節を読み直して節として書き直す
- **機械置換のターゲットではない**：末尾の点検フレーズや`prh-style.yml`の検出ルールは注意喚起であり、ヒットしただけで置き換えない
- **品質の最終判定は人の通読**：ルール準拠は二次評価で、一次評価は説明の前進と流れの自然さを人が判定する
...
- **「まとめ」は4点以内**にする。各点は事実1個に絞り、「次は〜で扱います」を入れる場合は1文だけにする
...
- 箇条書きの項目末は「である」調または体言止めに揃える。本文段落のです・ます調と同じファイル内に混在すると、textlintの`no-mix-dearu-desumasu`が混在エラーを出す。リード段落・解説段落の中で項目を並べるときも、項目末は「〜である」「〜する」「〜になる」、あるいは体言止めに寄せる
...
個々の比喩を残すか削るかは、次の3条件で判定します。比喩そのものを禁じる意図ではなく、説明の骨格を担う比喩は採用し、装飾としての比喩は削る、という方針です。3つすべてを満たすときに残します。

1. **目的が明示できる** — 何を理解させるための比喩か、一文で言える
2. **対応関係が一貫している** — 比喩の側のA・Bが、説明対象のA'・B'と1対1で対応し、文中で追跡できる
3. **本文の主張に橋を渡している** — 比喩の含意が、続く説明の補助になっている

採用してよい比喩の例：「LLMは生成AIの中核にあたる頭脳です。頭脳が大量のエネルギーを必要とするように、LLMの稼働にも大量の電力を消費します」。目的（LLMの位置づけと電力消費を同時に伝える）、対応関係（頭脳→LLM、エネルギー→電力）、橋渡し（次段の電力の話に直結）の3点が成立しています。
...
書き直しで比喩を消す前に、その比喩が3条件を満たして本文の流れを担っていないかを確認します。3条件を満たす比喩を機械的に削ると、対比構造や読者の足場まで失われることがあります。
...
- **「事情が出る」「結果の調整が利く」「仕組みの上に乗る」のように、作用点の見えない擬似動詞で説明を完結させない**。「事情」「調整」「乗る」は、書き手が頭の中で対象を漠然とまとめてしまった合図にあたる。何が・どう変わるか・なぜそうなるかを1度は文中で具体的に書き、そのうえで以後を圧縮する

注意：同じ語でも、技術用語として確立した用法（「テストが通る」「ビルドが通る」など）は固有名詞に近い扱いで残します。問題にしたいのは、概念の説明や読者の理解の進み方を比喩で代用している場合です。

このカテゴリの射程は「読者の認知や主観に介入する形」に限ります。「以降の章はこの前提に立って書いてあります」のような文書構造の客観的な説明は、対象外として残します。
...
地味 / 派手 / 華やか / 夢のある / 素直に / 意外と / ざっくり
潰す / 犯人探し / 誤爆 / 監督 / 肩代わり
ファイルを触れる / ファイルに触る / 腕と足 / 手と目 / 顔ぶれ / 兄弟 / ファミリー / 入口の性格
最後の手段 / 最後の選択肢 / せざるを得な / 温存する / 使うべきでない / できる限り避け
情シス / 情シス部門 / 情シス観点
結局いちばん / 覚えていただければ十分 / しましょう
理屈 / 事故る / 活きる / 活きにくい / 腹落ち / 早道 / ほぐす / ほぐし / 解きほぐ / 感覚を持って / 押さえておく / 把握しておく / 知っておく / 目を通しておく / 試しておく / 経験しておく / 体験しておく / 踏まえて読むほうが / 読み解きやすく / 読み進めやすくなり
事情が出 / 調整が利 / 仕組みの上に乗 / 上に乗せる / 差が出 / 特徴が出 / 必要が出てき / 要件が出
```
<!-- textlint-enable prh -->
