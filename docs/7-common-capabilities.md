...
ブランド違いのサービスを触っていると、細かな画面の違いに目が行きがちですが、**コア機能の顔ぶれはすでに似通っています**。本書では以下の3つを共通の柱として扱います。
...
最後の行のように、最近はテキスト以外の入力もチャットの延長で扱えます。**マルチモーダル**と呼ばれる領域で、Claude・Geminiの双方が画像・PDF・音声の読み取りを標準でサポートしています（動画は対応が広がりつつある段階です）。2026年時点では、スクリーンショットを貼って「ここの項目を表にして」と頼む運用が、もっとも費用対効果の高い使いどころです。
...
筆者の経験則では、「面倒だから一発で全部やらせよう」と長大な依頼文を投げた日のほうが、二度手間になる確率が高いです。チャットは対話。対話らしく、段階を踏むほうが結果的に早くなります。

## アーティファクト

チャットは流れていく会話ですが、業務ではしばしば「成果物として手元に残したいもの」が生まれます。議事録、スライドの下書き、コードの断片、プレビューできるHTML、などです。これを**会話の外側に置いて、別ペインで繰り返し編集できるようにする仕組み**を、本書では**アーティファクト**と呼びます。
...
- **実行環境はあくまでプレビュー** — 本運用を想定していない簡易サンドボックス。業務に乗せるなら、別途デプロイ先を用意する
- **バージョン管理は軽量** — 履歴をさかのぼれる機能はあるが、Gitほど堅牢ではない。重要な版は手元にも保存しておく

## コネクタ経由の外部サービス利用

3本目の柱は、ChatとArtifactだけでは届かない「外の情報・外の操作」に手を伸ばす機能です。仕組みは3章で扱ったツール呼び出しそのものですが、読者が普段触るのはたいてい**コネクタ**という形で抽象化された入口です。
...
**使い分けの直感を一言でまとめると、「Workspaceの内側はGeminiが近道、Workspaceの外側や横断利用はClaudeが強い」です**。ただし、コネクタの対応状況は月単位で変わるので、本書を読むときはかならず最新の公式情報を突き合わせてください。
...
この仕組みがわかっていると、セキュリティの感覚も自然に身につきます。「コネクタを1つ繋ぐ」とは、**その範囲の情報がAIのコンテキストに入る可能性を開くこと**です。情シス観点の整理は8章、エージェント運用まで踏み込んだ話は9章で扱います。
...
1本で閉じようとするほど、プロンプトは過剰に複雑になりがちです。**「これはどの柱の仕事か」と分解するだけ**で、見通しがかなり良くなります。
...
「まずチャット、必要ならアーティファクト、足りなければコネクタ」の順で拡張するのが、事故を防ぐ現実的な道です。
...
- **サービス間で同じプロンプトを使い回す** — ClaudeとGeminiでは既定のシステムプロンプトや振る舞いが違う。「この書き方はClaudeでは効くがGeminiではブレる」といった差は地道に観察しておく
- **アーティファクトを本番環境代わりに使う** — あくまでプレビュー用途。本運用へ出す前に、本来のホスティング先へ必ず移す
...
- 「どの柱の仕事か」を分解してから手を動かすと、プロンプトと運用の両方が軽くなる
