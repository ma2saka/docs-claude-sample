...
本章では上の表のうち、Geminiアプリと、Workspaceのサイドパネルの2系統だけを扱います。Gemini in Chromeは2026年4月に日本でも提供が始まった、Chromeブラウザに直接組み込まれたGeminiです。機能と利用条件の詳細は[11章](11-gemini-advanced.md)で扱います。モバイルアプリは、本章を終えたあとに必要に応じて試してください。
...
新しいチャットでは前の会話が引き継がれない、という挙動は、以降の章で扱う「コンテキスト」の説明の出発点にあたります。以降の章は、この挙動を一度は試した前提で書いてあります。
