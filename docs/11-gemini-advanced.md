...
2026年5月のGoogle I/Oでは、テキスト・音声・画像・動画を混在させた入力から動的な映像を生成・編集する新モデル **Gemini Omni** が発表されました。まず開発者・法人向けに「Gemini Omni Flash」がGemini API経由で順次提供される見込みです。対応状況は参考欄の公式情報を確認してください（最終確認：2026-06-08）。
...
[8章](08-common-capabilities.md)では、両サービスに共通する軽量・標準・重量級の3段構造を整理しました。Geminiでは現時点で、応答までの時間が短いFlash系と、難問の精度を引き上げるPro系という形でモデルが並びます。2026年5月のGoogle I/Oで、エージェント処理とコーディングに強い **Gemini 3.5 Flash** の一般提供が始まり、Flash系の現行版にあたります。上位の **Gemini 3.5 Pro** は同時に発表され、続いて提供される位置づけです（最終確認：2026-06-08）。さらに、回答前に内部で長めに考える**Deep Think**を選択できる場面が用意されています。Deep Thinkは個人向けの **Google AI Ultra** サブスクで利用でき、Pro／Flashと並ぶ形で、思考量を多めに取る選択肢として加わります。Workspaceの標準プランで扱えるGemini Pro／Flash系統とは、利用範囲が分かれています（最終確認：2026-05-16）。多段の数学・コード生成・厳密な手順の組み立てといった題材で精度が上がりやすい一方、応答時間と費用は通常の応答より増えます（推論モード／思考モードの位置づけは[2章](02-what-is-generative-ai.md)・[7章](07-terminology.md)を参照してください）。利用可否はプランやUIによって異なるため、参考欄の公式ヘルプで都度確認してください。
