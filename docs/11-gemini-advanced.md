...
- Geminiを業務で何度か利用しており、機能ごとの使い分けの軸が欲しい人
- ClaudeとGeminiの違いを、機能と制約の両面から把握したい人

各機能の仕様・料金・プランによる利用可否は、四半期単位で変わります。本章では機能の意味と選び方に集中し、最新の詳細は各参考URLで確認してください。
...
- 口調を都度指定する。読者層（社内向け／社外向け）と文体（です・ます／箇条書き中心など）を書いておくと、出力のばらつきを抑えられる
...
参照させたい資料が数十本以上の規模になると、Gemsのナレッジでは手動の管理が難しくなります。その場合の選択肢になるのが、RAG（検索拡張生成）と呼ばれる仕組みです。RAGの概念と用語の対応は[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で扱っています。
...
[13章](13-claude.md)で扱うClaudeのメモリ機能と役割が近く、どちらも「毎回同じ前提を伝え直す手間」を省くための仕組みです。[5章](05-misunderstanding-learning.md)で見たとおり、これはモデル自体が学習しているのではなく、保存された情報を毎回コンテキストに添えて渡す設計です。
...
Gemsと保存済み情報は、適用範囲と明示性が異なります。[13章](13-claude.md)で扱うClaudeのProjectsとの対比もあわせて並べます。
...
Google Workspaceの管理対象アカウントでは、保存済み情報の利用可否が管理者の設定に従います。管理者がGemini Apps Activityを無効にしている場合、保存済み情報の機能自体は利用できません。設定メニューに該当項目が表示されない場合は、組織のWorkspace管理者による機能制限が原因です（最終確認：2026-05-25）。
...
Geminiのチャットでも、CSVファイルなどのデータを添付して集計やグラフ作成を依頼すると、内部でコードを自動実行して結果を返す機能を利用できます。利用者がコードを書く必要はなく、「月別に集計してグラフにして」のような依頼で使えます。テキストだけで数値を処理させる場合と比べ、計算の正確さが上がりやすくなります。この能力の共通的な見方は[8章](08-common-capabilities.md)の「コード実行によるデータ分析」で取り上げています（最終確認：2026-05-27）。

## マルチモーダルは画像・PDF・音声・動画・YouTubeリンクを混在させて渡せる

Geminiは2026年時点で、テキスト以外の入力として以下が実用的に使えます。これらを業務で扱うときの共通の枠組み（4系統の素材と注意点）は[3章（マルチモーダル）](03-multimodal.md)で説明しており、本節は、その枠組みに対するGemini側の具体像を示します。
...
複数の素材を組み合わせて1回の依頼に渡せます。「この契約書PDFとこの会議メモ画像をもとに、論点を整理して」のような依頼が一度で済みます。素材が多いほど文脈が豊かになる一方、コンテキストウィンドウの上限（[7章](07-terminology.md)）に近づくのが早まります。全部まとめて渡す前に、まず要点だけ渡して結果を確認してから追加する進め方のほうが、応答が安定しやすくなります。
...
Deep Researchと通常のチャットは、呼び出し画面は同じGeminiアプリですが、動きの構造が異なります。
...
Deep Researchの出力には、参照したWebページへのリンクが付きます。ただし、リンクが付いていることと内容の正確性は別の問題です。引用元の記述が古い、誤っている、文脈から切り離されている場合があるため、業務で使う前に一次ソースを開いて裏取りを行う手順は、通常のチャットと同じです（裏取りの考え方は[6章](06-hallucination-and-knowledge-literacy.md)で扱っています）。

公開情報をもとにした調査レポートという用途では、Claude側にも同じ系統の機能としてResearchがあります（[13章](13-claude.md)）。Deep Researchが公開Webの情報を素材にするのに対し、ClaudeのResearchはGoogle Workspaceアカウントを接続すれば社内のメール・ドキュメント・カレンダーも調査対象に含められる点が、両者の主な違いです。応答時間の目安や対比表は13章にまとめてあります。

### 利用条件

Deep Researchは、個人向けのGoogle AI Ultraサブスク、およびGoogle One AI Premiumで利用できます。Workspaceの標準プランでは利用できない場合があります。利用可否はプランにより異なるため、参考欄のヘルプで確認してください（最終確認：2026-05-25）。
...
精度と費用の特徴は次のとおりです。多段の数学・コード生成・厳密な手順の組み立てといった題材で精度が上がりやすい一方、応答時間と費用は通常の応答より増えます（推論モード／思考モードの定義は[2章](02-what-is-generative-ai.md)・[7章](07-terminology.md)を参照してください）。利用可否はプランやUIによって異なるため、参考欄の公式ヘルプで都度確認してください。
...
Gemini Liveは複数の画面から起動できます。画面によって、会話に渡せる材料が異なります。
...
- 移動中や作業中に手が離せないとき — キーボードを使わずに情報の確認や見直しができる
- アイデアの壁打ち — 口頭で考えを話しながら、Geminiに論点の集約や反論の観点を求める
...
2026年4月に日本でも提供が始まった**Gemini in Chrome**は、Chromeブラウザに直接組み込まれたGeminiです。Geminiアプリやサイドパネルへ移動せず、いま開いているタブをそのまま材料として会話できます。Geminiアプリ／Workspaceアプリのサイドパネル／Gemini in Chromeの3つは同じGeminiを呼び出す画面で、向く場面が異なります。
...
**Auto browse**（自動ブラウジング）は、目的を伝えるとGeminiが複数のページを横断して情報収集や入力を進める動作モードです。旅行プランの比較、フォーム入力、定期購入の見直しなどが対象に挙げられます。購入確定や投稿のような確認の必要な操作の前では、Geminiが利用者に確認を求めて停止する動作になっています。2026年4月時点では米国のGoogle AI Pro／Ultra加入者に対するプレビュー提供で、日本での提供時期は未定です。最新状況は参考欄のヘルプで確認してください。
...
Gemini in Chromeの利用条件とデータの取り扱いは、Chromeにログインしているアカウントの種類によって変わります。以下は2026年4月時点の概略です。
...
Workspaceで利用する場合、Geminiアプリ設定そのものが無効化されていると、Chrome側のサイドパネル自体が出てこないことがあります。会社のChromeで「Geminiに相談」ボタンが見当たらないときは、利用しているアプリの不具合ではなく、組織の管理ポリシーで無効化されている、もしくは個別の有効化設定が反映されていないことが大半です。プランと有効化状況は、組織のWorkspace管理者または契約・プランの担当窓口に確認してください。
...
### ChromeのAI機能はクラウドで動く層と端末内で動く層に分かれる

ここまで扱ったサイドパネルのGemini in Chromeは、タブの内容や依頼文をGoogleのサーバへ送り、クラウド側のGeminiで処理する仕組みです。一方、Chromeというブラウザ自体には**Gemini Nano**と呼ばれる小型のモデルが同梱されており、一部のAI処理は手元の端末の中だけで完結します。同じ「ChromeのAI機能」でも、データが処理される場所はこの2層に分かれます。

| 層 | 呼び出され方 | データの処理場所 |
| ---- | ---- | ---- |
| クラウドのGemini | サイドパネル（Geminiに相談）、複数タブの比較、Auto browse | タブの内容や依頼文がGoogleのサーバへ送られて処理される |
| 端末内のGemini Nano | Webページや拡張機能が、要約・翻訳・文章補助の部品として呼び出す | 初回にモデルを端末へダウンロードしたあと、処理は端末内で完結する |

Gemini Nanoには、サイドパネルのように利用者が直接会話する画面はありません。Webサイトや拡張機能の側が、**組み込みAI**（Built-in AI）と呼ばれるChromeの仕組みを通じて呼び出し、その結果が個々の機能の動作として現れます。2026年6月時点では、要約・翻訳・言語判定・自由な指示の実行までが安定版のChromeから呼び出せる段階に入り、日本語も入出力の対応言語に含まれています。文章の生成・校正に特化した仕組みは試験提供の段階です。

端末側の動作条件の概略は次のとおりです。Gemini in Chromeが地域と言語で提供範囲を区切るのに対し、Gemini Nanoの利用可否はOSとハードウェアの条件で決まります（最終確認：2026-06-12）。

- 対応OSはWindows 10/11、macOS 13以降、Linux、ChromeOS（Chromebook Plus）。モバイル版Chrome（Android・iOS）は対象外である
- モデルの取得には、Chromeのプロファイルがあるドライブに22GB以上の空き容量が必要になる（モデル本体は3〜4GB程度）
- GPUのメモリ（VRAM）4GB超が基本の要件で、Chrome 140以降はメモリ16GB以上・4コア以上のCPUでも実行できる
- 初回のダウンロード後は、ネットワークに接続していなくても動作する

この区別は、[9章](09-security-individual.md)で示した「どこに何を渡したか」の判断に直結します。クラウドのGeminiへ送る処理と端末内で完結する処理とでは、機密情報を扱うときの前提が変わるためです。ただし、画面上のある機能がどちらの層で動いているかは、利用者の側から見分けにくい場面が残ります。判断に迷う場合は、クラウドへ送られる前提に立って、9章の基準を当てるほうが安全側の判断になります。また、推論を担うコンピュータが手元の端末にあるという性質は、[Appendix: ローカルLLM](appendix-local-llm.md)で扱う「推論場所」の軸の身近な実例です。

### サイドパネル以外のAI機能は設定の「AIイノベーション」に並ぶ

Chromeの「設定」→「AIイノベーション」には、サイドパネル以外のAI機能がまとまっています。代表的なものを並べます。

- 履歴のAI検索 — 自然な文（「先週見た料金比較のページ」など）で閲覧履歴を探せる。利用者が有効化した時点から索引が作られる
- タブ整理 — 開いている多数のタブから、まとめられそうなグループを提案する
- 文章作成サポート — Webページの入力欄を右クリックして、下書きの生成や書き直しを依頼できる
- AIテーマ — ブラウザの外観テーマを説明文から生成する

2026年6月時点では、これらの多くが米国かつ英語設定のChromeに限定されており、日本語環境のChromeでは利用できないものが多い状況です。また、個々の機能がクラウドと端末内のどちらの層で動くかは機能ごとに異なり、公開情報からは判別しにくい場合があります。機能の並びと提供条件は版ごとに入れ替わるため、設定画面の表示と参考欄の公式ヘルプで確認してください（最終確認：2026-06-12）。
...
- 音声で方針確認や論点の洗い出しを進めたい場合 — Gemini Liveがアプリ・モバイル・Chromeの3つの画面から音声対話を提供する
...
- マルチモーダル入力に加え、画像生成（Imagen）・動画生成（Veo）・公開Webを段階的に調べるDeep Researchが、Geminiアプリのチャットから利用できる。Deep Researchの出力は下書き扱いで、一次ソースでの裏取りが前提になる
- Gemini Live（音声対話モード）とGemini in Chrome（ブラウザの組み込み画面）が、チャット画面とは別の呼び出し画面として加わる。Chromeには端末内のGemini Nanoで完結する処理層もあり、自動メール送信・任意のMCPサーバへの直接接続・Workspace統合のプラン依存の3制約は、本章末尾の節で詳しく扱った
