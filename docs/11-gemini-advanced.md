...
本章は、Gemini固有の機能と、2026年5月時点で利用者が押さえておきたい制約、Gemini側に寄せたほうが進めやすい場面の3点を扱います。取り上げる機能は、Gems、保存済み情報、Canvas、マルチモーダル入力、画像生成（Imagen）、動画生成（Veo）、Deep Research、Gemini Liveの8つです。加えて、Chromeブラウザに組み込まれた呼び出し口であるGemini in Chromeと、Chromeに同梱されたオンデバイスモデルGemini Nanoとの関係も扱います。[8章](08-common-capabilities.md)で整理した「チャット／アーティファクト／コネクタ」の共通骨格に対する、Gemini側の具体像にあたる位置づけです。
...
この区別は、[9章](09-security-individual.md)で整理した「どこに何を渡したか」の判断に直結します。クラウドのGeminiへ送る処理と端末内で完結する処理とでは、機密情報を扱うときの前提が変わるためです。ただし、画面上のある機能がどちらの層で動いているかは、利用者の側から見分けにくい場面が残ります。判断に迷う場合は、クラウドへ送られる前提に立って9章の基準を当てる扱いが安全側になります。また、推論を担うコンピュータが手元の端末にあるという性質は、[Appendix: ローカルLLM](appendix-local-llm.md)で整理する「推論場所」の軸の、身近な実例にあたります。

### サイドパネル以外のAI機能は設定の「AIイノベーション」に並ぶ

Chromeの設定画面には「設定」→「AIイノベーション」という項目があり、サイドパネル以外のAI機能がここにまとまっています。代表的なものを並べます。
...
- Gemini LiveはGeminiアプリ・モバイルアプリ・Gemini in Chromeの3入口から起動できる音声対話モードで、壁打ちや方針確認に向く。Gemini in Chromeは現在のタブを材料に呼び出せる入口で、複数タブの比較・Auto browse（米国でプレビュー）まで含む。ChromeのAI機能には、クラウドのGeminiへ送って処理する層と、端末内のGemini Nanoで完結する層の2層がある。自動メール送信・任意のMCPサーバへの直接接続・Workspace統合のプラン依存の3点は、2026年春時点の境界として残る
