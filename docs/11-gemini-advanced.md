...
各機能の仕様・料金・プランによる利用可否は、四半期単位で変わります。本章では機能の意味と使い分けに集中し、最新の詳細は各参考URLで確認してください。
...
Geminiは2026年時点で、テキスト以外の入力として以下が実用的に使えます。これらを業務で扱うときの共通の枠組み（4系統の素材と注意点）は[3章（マルチモーダル）](03-multimodal.md)で扱っており、本節は、その枠組みに対するGemini側の具体像にあたります。
...
この区別は、[9章](09-security-individual.md)で示した「どこに何を渡したか」の判断に直結します。クラウドのGeminiへ送る処理と端末内で完結する処理とでは、機密情報を扱うときの前提が変わるためです。ただし、画面上のある機能がどちらの層で動いているかは、利用者の側から見分けにくい場面が残ります。判断に迷う場合は、クラウドへ送られる前提に立って、9章の基準を当てるほうが安全側の判断になります。また、推論を担うコンピュータが手元の端末にあるという性質は、[Appendix: ローカルLLM](appendix-local-llm.md)で扱う「推論場所」の軸の身近な実例にあたります。
