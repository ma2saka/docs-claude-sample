...
Geminiでは現時点で、応答までの時間が短いFlash系と、難問の精度を引き上げるPro系という形でモデルが並びます。さらに、回答前に内部で長めに考える**Deep Think**を選択できる場面が用意されています。2026年2月にリリースされた **Gemini 3 Deep Think** がこのDeep Thinkの現行世代にあたり、Pro／Flash と並ぶ形で、内部の思考量を多めに取るモードとして加わります。Gemini 3 Deep Think は個人向けの **Google AI Ultra** サブスクで利用でき、Workspace の標準プランで扱える Gemini Pro／Flash 系統とは利用範囲が分かれています（最終確認：2026-05-16）。多段の数学・コード生成・厳密な手順の組み立てといった題材で精度が上がりやすい一方、応答時間と費用は通常の応答より増えます（推論モード／思考モードの位置づけは[2章](02-what-is-generative-ai.md)・[7章](07-terminology.md)を参照してください）。利用可否はプランやUIによって異なるため、参考欄の公式ヘルプで都度確認してください。
