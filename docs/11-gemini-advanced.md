...
- Gemini in ChromeはChromeのサイドパネルから現在のタブを材料に呼び出せる入口で、複数タブの比較・Gemini Live・Auto browse（米国でプレビュー）まで含む。Workspace環境では管理者の有効化設定に従う。自動メール送信・任意のMCPサーバへの直接接続・Workspace統合のプラン依存は2026年春時点の境界として残る
