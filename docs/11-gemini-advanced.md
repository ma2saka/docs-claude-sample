...
本章は、Gemini固有の機能と、2026年4月時点で利用者が押さえておきたい制約、Gemini側に寄せたほうが進めやすい場面の3点を扱います。取り上げる対象は、Gems、保存済み情報、Canvas、マルチモーダル入力、画像生成（Imagen）、動画生成（Veo）、Deep Researchの7つの機能と、Chromeブラウザに組み込まれた呼び出し口であるGemini in Chromeです。8章で整理した「チャット／アーティファクト／コネクタ」の共通骨格に対する、Gemini側の具体像にあたる位置づけです。
...
### Gemsとの違いとClaudeのProjectsとの対比

Gemsと保存済み情報は、適用範囲と明示性が異なります。13章で扱うClaudeのProjectsとの対比もあわせて整理します。
...
Google Workspaceの管理対象アカウントでは、保存済み情報の利用可否が管理者の設定に従います。管理者がGemini Apps Activityを無効にしている場合、保存済み情報の機能自体が利用できないことがあります。自分のアカウントで設定メニューに該当項目が表示されない場合は、組織のWorkspace管理者が機能を制限している状態にあたります（最終確認：2026-05-25）。
