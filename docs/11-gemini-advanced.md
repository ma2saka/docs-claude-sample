...
- Geminiを業務で何度か利用しており、機能ごとの使い分けの軸を持ちたい人
...
参照させたい資料が数十本以上の規模になると、Gemsのナレッジでは手動の管理が難しくなります。その場合はRAG（検索拡張生成）と呼ばれる仕組みが選択肢に入ります。RAGの概念と用語の対応は[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で扱っています。
