...
「これ、AIに貼っても大丈夫ですかね？」と、勤め先のチャットで誰かが尋ねる場面を、あなたも一度は見かけたことがあるはずです。ここまでの章で「学習」やハルシネーションといった怖がられがちな論点はかなり整理してきたので、本章ではもう一段、**普段の業務でツールを手に取るとき、自分の手元で何を気にすればよいか**を取り上げます。

本章の前提はシンプルです。読者は組織のガバナンスを設計する立場ではなく、ただ仕事を前に進めたい一人のユーザーです。「気をつけるべきことが多すぎて、結局何もできない」という状態にならないよう、論点を3つの箱に分けて、順に手渡していきます。組織側がなぜルールを敷くのかという背景は、続く[10章](10-security-agent-era.md)の冒頭で扱います。

## 対象読者と前提

- [1章](01-gemini-in-workspace.md)や[8章](08-common-capabilities.md)で、ClaudeやGeminiを業務で触った経験がある人
- [5章（「学習」というキーワードの誤解）](05-misunderstanding-learning.md)で、入力データがモデル本体に焼き付くような挙動は普段起きない、という話に目を通した人
- 出力を受け取ったあとの裏取りは[6章（ハルシネーション）](06-hallucination-and-knowledge-literacy.md)で扱い済み。本章では「受け取ったあと、どう扱うか」に集中する

本章のゴールは、入力・出力・履歴の3つの箱を、自分の手元で迷いなく扱えるようになることです。ルールを丸暗記するよりも、3つの箱で何が起きうるかを構造として押さえておくほうが、現場で判断がつきやすくなります。
...
3つのうち1つでも「うっすら引っかかる」なら、貼り付ける前に一呼吸置く合図だと思ってください。実務でよく事故が起きるのは「機密度は大丈夫そうだが、他社の提供資料だった」というケースです。3つ全部を毎回意識すると、この種の取りこぼしを避けやすくなります。
...
「業務アカウントと個人アカウントを混同しない」のは、事故防止の効き目が大きい割に、誰でも今日からできる切り口です。[5章](05-misunderstanding-learning.md)で触れたとおり、無料／個人向けUIとビジネス向けUIで、入力データの既定の扱いに差があるためです。
...
出力側で注意したい点は、[6章](06-hallucination-and-knowledge-literacy.md)で扱った「事実の裏取り」とは別レイヤーです。本章で扱うのは、**受け取った出力を、次にどこへ転記するか**の部分です。
...
[8章](08-common-capabilities.md)で触れたとおり、Claude ArtifactsやGemini Canvasでは、作った成果物に共有リンクを発行できます。便利ですが、**デフォルトの公開範囲と共有後の寿命**は、サービスごとに挙動が違います。
...
[5章](05-misunderstanding-learning.md)で分解したとおり、この3つは**モデル本体の重みには触らない**領域です。その意味では「学習されていない」は正しいのですが、**自分のメモと会話が、サービス側に残っている**こと自体は変わりません。
...
- 組織側がなぜルールを敷くのか、その背景と、エージェント時代に増える論点は [10章（セキュリティ エージェント時代のガバナンス）](10-security-agent-era.md) で扱う
