...
- **自動承認モードは必要な範囲に絞る** — 全部を無条件に通す設定は、砂場の囲いを踏み潰すのと同じ

付録「[Claude Code](appendix-claude-code.md)」でも触れた「小さなフォルダで、人の目を通しながら、徐々に広げる」という運用は、サンドボックスの具体的な実践そのものです。

### 組織がサンドボックスを厚くする理由

社内で「このツールはこのプロジェクトだけ」「この操作には別途承認を取る」というルールに出くわすと、遠回りに感じる場面があります。背景にあるのは、エージェントの失敗の**ブラストラディウス**（事故が及ぶ範囲）を小さくしたい、という発想です。

同じ失敗でも、閉じた砂場の中ならやり直せます。本番データや共有ドライブに直接触っていると、取り返しがつきません。利用者の目線で言い直せば、「**やらかしてもいい場所で遊ぶ**」という安全余地を、組織がわざわざ用意してくれているだけ、という話です。
...
この3つは、エージェント固有の話というより、アカウント管理の基本です。ただ、エージェント時代は**鍵束を持たせる頻度が一気に上がる**ぶん、基本を飛ばすと事故の密度も上がります。
...
エージェント時代の3つ目の論点は、もう少し地味で、もう少し深刻です。**生成のスピードと量に、人間のレビューが物理的に追いつかない**という問題です。

### 数字の肌感覚

一人のユーザーが1日に目を通せる文章量は、そう大きく変わりません。一方で、エージェントは並列に走り、数十のファイルを短時間で書き換え、数百の提案を一晩で積み上げます。**生産量と確認量の差は、指数関数ではなく単に掛け算で広がります**。気づいた頃には、未レビューの変更の山を前に立ち尽くす展開が待っています。
...
自動チェック（lint、テスト、スキャナ）は、人間の目を置き換えるものではありません。**人間の目が見るべき場所を絞り込むためのふるい**です。ふるいを通ったものだけを人が見る、という割り切りで、やっとスケールが釣り合います。
...
「AIが書きました」で責任が減る場面はありません。量が増えたぶん、**残す証跡が薄くならないようにする**ほうが現実的な落としどころです。
...
- **社内ルールを「面倒なだけ」と捉える** — サンドボックスや権限分離は、利用者の失敗コストを**組織側が肩代わりしている**しくみでもある

最後の項目は、8章のまとめと同じ趣旨です。個人利用であれエージェント時代であれ、**ルールの裏にある理屈を掴んでおく**ほうが、結果として大胆に道具を使えます。

## 次の章への橋渡し

ここまでで、セキュリティの地図はひと通り描けました。次章以降は、個別のツールを深く使いこなす話題に戻ります。Claudeを主役にする12章では、本章で扱ったサンドボックスや権限の考え方が、そのままClaudeの使いこなしと地続きだと分かるはずです。

- [10章（あらためてGeminiを使いこなそう）](10-gemini-advanced.md) — Gemini固有の手ざわりを、セキュリティの前提を踏まえて整理する
- [付録: Claude Code](appendix-claude-code.md) — サンドボックスの具体像を、Claude Codeの運用で掴む

セキュリティは、道具を制限するための話ではなく、**安心して大胆に道具を使えるようにするための下地**です。ここまでの8・9章は、後続の章を「遠慮なく踏み込む」ために敷いたマットだと思ってください。
