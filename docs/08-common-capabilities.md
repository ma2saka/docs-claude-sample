...
おおまかな目安として、Workspaceの内側やブラウザで開いているWebページを材料にする作業はGemini、Workspace外のSaaS連携や独自ツールとのMCP連携はClaudeに寄せます。公開情報を時間をかけて調べるレポート型の作業もGemini側に寄ります。もう少し粒度を上げると次のように並べられます（最終確認：2026-05-25）。
...
| テーマを決めて公開情報から調査レポートをまとめたい | Gemini | Deep Researchが複数のWebソースを段階的に探索し、引用元つきのレポートにまとめる（[11章](11-gemini-advanced.md)） |
