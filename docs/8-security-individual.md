...
本章の前提はシンプルです。読者は情シスや法務の担当者ではなく、ただ仕事を前に進めたい一人のユーザーです。「気をつけるべきことが多すぎて、結局何もできない」という状態にならないよう、論点を3つの箱に分けて、順に手渡していきます。

## 対象読者と前提

- [1章](1-gemini-in-workspace.md)や[7章](7-common-capabilities.md)で、ClaudeやGeminiを業務で触った経験がある人
- [4章（「学習」というキーワードの誤解）](4-misunderstanding-learning.md)で、入力データがモデル本体に焼き付くような挙動は普段起きない、という話に目を通した人
- 出力を受け取ったあとの裏取りは[5章（ハルシネーション）](5-hallucination-and-knowledge-literacy.md)で扱い済み。本章では「受け取ったあと、どう扱うか」に集中する

本章のゴールは、情シスや法務がなぜ一部の使い方にブレーキをかけるのか、その**仕組み側の理屈**を読者自身がたどれるようになることです。ルールを丸暗記するよりも、理屈で分かっていたほうが、現場で判断がつきやすくなります。
...
3つのうち1つでも「うっすら引っかかる」なら、貼り付ける前に一呼吸置く合図だと思ってください。実務でよく事故るのは「機密度は大丈夫そうだが、他社の提供資料だった」というケースです。3つ全部を毎回意識すると、この種の取りこぼしが減ります。
...
「業務アカウントと個人アカウントを混同しない」のは、事故防止の効き目が大きい割に、誰でも今日からできる切り口です。[4章](4-misunderstanding-learning.md)で触れたとおり、無料／個人向けUIとビジネス向けUIで、入力データの既定の扱いに差があるためです。
...
機密を「そのままの姿で全部渡す」必要があるケースは、実はあまりありません。固有名詞を仮名に置き換える、金額の桁だけを残す、といった軽いマスキングで済む場面がほとんどです。**渡す情報を削るのは、セキュリティだけでなく回答の質のためにも役立ちます**。余分な情報はモデルの注意を散らかすからです。

## 出力: 返ってきたものをどう扱うか

出力側で注意したい点は、[5章](5-hallucination-and-knowledge-literacy.md)で扱った「事実の裏取り」とは別レイヤーです。本章で扱うのは、**受け取った出力を、次にどこへ転記するか**の部分です。
...
[7章](7-common-capabilities.md)で触れたとおり、Claude ArtifactsやGemini Canvasでは、作った成果物に共有リンクを発行できます。便利ですが、**デフォルトの公開範囲と共有後の寿命**は、サービスごとに挙動が違います。
...
3つ目の箱が、いちばん静かで、いちばん事故りやすい領域です。AIは忘れてくれるとは限りません。**「モデルは忘れていても、サービスは覚えている」**が基本線だと思ってください。
...
[4章](4-misunderstanding-learning.md)で分解したとおり、この3つは**モデル本体の重みには触らない**領域です。その意味では「学習されていない」は正しいのですが、**自分のメモと会話が、サービス側に残っている**こと自体は変わりません。
...
## 情シスが気にしていることと、利用者の目線

社内で「このツールは禁止」「この使い方だけ許可」というルールに出くわすと、利用者側からは窮屈に見える場面があります。情シスが何を心配しているかを、利用者の目線で翻訳しておくと、社内ルールと折り合いをつけやすくなります。

| 情シスの心配 | 利用者の目線での意味 |
| ---- | ---- |
| 機密情報が外部サービスに流出しないか | どのツールに貼り付けてよいかが、アカウントごとに決まっている |
| 契約で禁じられた使い方をしていないか | 無料プランの便利機能でも、社内の契約で使えない場合がある |
| ログや監査の要件を満たせるか | 誰がいつ何を入力したかを、会社としてたどれる必要がある |
| 退職時にデータが人と一緒に出ていかないか | 業務アカウントで作ったものは、個人の持ち物ではない |

このリストを見ると、「情シスは意地悪で止めているわけではない」ということが見えてきます。止めているのは、**事故の起点になりやすい経路**です。禁止の理由を理屈で掴んでおくと、「じゃあこの経路ならよいですか？」と建設的に相談ができます。
...
ガイドラインはしばしば「面倒な文書」の顔で届きますが、ほとんどの会社で**利用者を守るための安全帯**として書かれています。ルールを知っているほうが、むしろ大胆に道具を使える、というのが実務のパラドックスです。
...
5つ目は、作業のたびに毎回やる必要はありません。機微情報を扱った日だけでも、定着するとかなり効きます。

## よくある失敗パターン

- **個人アカウントで社内資料を要約してしまう** — 業務アカウントとの切り替え忘れ。ブラウザのプロファイルを分けると構造的に防げる
...
ここまでは、あくまで**一人の利用者の手元**に閉じた論点でした。エージェントを走らせたり、MCP経由で複数サービスをつないだり、人の目が追いつかない速度で出力が増えたりする世界では、別種の論点が出てきます。

- [9章（セキュリティ エージェント時代のガバナンス）](9-security-agent-era.md) — サンドボックス、MCP／コネクタの境界、レビューが追いつかない問題

「個人で気をつけること」と「組織で気をつけること」は地続きですが、扱う道具も違います。本章の3つの箱を押さえたうえで、次章に進んでいただくと、見通しが一段上がります。
...
- 情シスのルールは、利用者を縛るためでなく**事故経路を塞ぐため**にある。理屈で納得してから付き合う
