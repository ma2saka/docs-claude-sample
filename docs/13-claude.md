...
Researchは、内部で複数のエージェントを並列に動かすマルチエージェント構成で調査を進めます。利用者から見える流れは次のとおりです。

1. 利用者が調査テーマを入力する
2. リードエージェントがテーマを分析し、調査計画を組み立てる
3. 複数のサブエージェントが計画に沿って並列にWebを検索・探索する
4. リードエージェントがサブエージェントの結果を統合し、引用元つきのレポートにまとめて返す
