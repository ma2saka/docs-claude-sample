...
本章は、Claude固有の機能と、2026年4月時点で利用者が押さえておきたい論点、Claude側に寄せたほうが進めやすい場面の3点を扱います。取り上げる機能は、Projects、Artifacts、画像生成、コネクタ、Web検索、Research、リモートMCPサーバ、Claude Cowork、メモリ、スタイルの10個です。[8章](08-common-capabilities.md)で整理した「チャット／アーティファクト／コネクタ」の共通骨格に対する、Claude側の具体像にあたる位置づけです。
...
Artifactsにはコード実行によるデータ分析の機能も含まれます。CSVファイルを添付して集計やグラフ作成を依頼すると、ClaudeがPythonコードを内部で生成・実行し、計算結果やグラフ画像をArtifacts上に返します。利用者がコードを書く必要はありません。テキストだけで数字を処理させる場合と比べ、コードが計算を担う分だけ集計の正確さが上がります。この能力の共通的な位置づけは[8章](08-common-capabilities.md)の「コード実行によるデータ分析」で整理しています（最終確認：2026-05-27）。
...
画像の入力（アップロードした画像の読み取り・分析）は以前から対応しており、[3章](03-multimodal.md)で扱ったマルチモーダル入力の一部です。本節で扱うのは出力側、つまりテキスト指示から画像を生成する機能にあたります（最終確認：2026-05-27）。
...
[11章](11-gemini-advanced.md)で扱ったGeminiの保存済み情報と役割が近く、どちらも個人の好みや事実情報を会話をまたいで保持し、毎回同じ前提を伝え直す手間を省く仕組みです。Geminiが保存済み情報とGemsの2系統で構成されるのに対し、Claudeは本章のProjects・メモリ・スタイルの3系統に分かれ、本節のメモリはそのうち個人向けの1つにあたります。
...
[8章](08-common-capabilities.md)では、両サービスに共通する軽量・標準・重量級の3段構造を整理しました。Claudeは現時点で、Opus／Sonnet／Haikuの3系統のモデルが並びます。重い推論はOpus、日常的な利用はSonnet、軽量で件数の多い処理はHaiku、というおおまかな住み分けです。2026年5月には **Opus 4.8** が一般提供（GA）に至り、Opus系統の現行版にあたります（最終確認：2026-06-11）。さらに2026年6月には、Opusの上位ティアとして **Mythosクラス**（一般提供版はFable 5）が加わりました。3系統の構成の上にもう1段重なる位置づけで、利用料金は上位ほど上がる設計のため、用途と費用の釣り合いから選びます。Projectsやコネクタの裏側でも、状況に応じてモデルが切り替わる場面もあります。同じ質問への回答の傾向が以前と異なる場合は、モデル選択の指定欄を確認してください。
...
- Artifactsは動くプレビューと公開リンクの2点でCanvasと違いがある。提案や勉強会など、相手に動作を見せる局面で適用しやすい。画像生成はチャット画面から呼び出せる。方向性を素早く試す用途に向き、対話で段階的に調整できる
...
- プラン差・コネクタの鮮度・モデル選択（Opus／Sonnet／Haikuの3系統と上位のMythosクラス）は、半年単位で内容が変わる前提で扱う
