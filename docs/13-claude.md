...
4製品はいずれも同じClaudeのモデル群（Opus／Sonnet／Haiku）を呼び出します。回答の傾向や言語表現の品質そのものは、同じモデルを選んでいる限り大きな差は出にくい一方、システムプロンプト、利用できるツール、コンテキストに入る情報源は製品ごとに異なります。出てくる応答の中身も、その違いの分だけ変わります。製品間のもうひとつの違いは利用者のPCに対する操作権限の範囲で、表の下に進むほど、読み書きできるファイルや実行できる操作の幅が広がります。習得にかかる時間と、想定外の操作が起きたときの影響範囲も同じ順序で大きくなるため、選ぶときは「どこまで任せたいか」を先に決めると、判断の軸が揃います。
...
仕組みそのものは4章で扱った[ツール呼び出し](04-external-system-integration.md)にあたり、ClaudeはMCP（Model Context Protocol）経由でこれらを参照しています。利用者の操作はコネクタを有効化するだけですが、裏側で4章の流れが動いていると把握しておくと、出力が想定と異なるときの切り分けがしやすくなります。
...
各モデルは、応答の前にモデル内部で推論ステップを重ねる**Extended Thinking**（拡張思考）モードに対応します。最近のモデルでは、依頼の難しさに応じて推論の深さが自動で調整される動きも入っています。多段の推論や、コードや数式の組み立てを伴う依頼で、このモードを有効にしておくと、応答の精度を安定させやすくなります。応答までの時間とトークン消費は、Extended Thinkingを使わない場合より増えます。
...
- プラン差・コネクタの鮮度・モデル選択（Opus／Sonnet／Haiku）は、半年単位で内容が変わる前提で扱う。Claude Codeやデスクトップ自動化までの拡張は[Appendix: Claude Code](appendix-claude-code.md)・[Appendix: デスクトップの自動化](appendix-desktop-automation.md)を参照
