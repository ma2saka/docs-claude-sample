...
[11章](11-gemini-advanced.md)で扱ったGeminiの保存済み情報がGemsと並ぶ2系統で構成されるのに対し、Claudeは本章のProjects・メモリ・スタイルの3系統に分かれます。本節のメモリは、そのうち個人向けの1系統に当たります。
...
**スタイル**（Styles）は、応答のトーンや形式を利用者が明示的に選択できる仕組みです。チャット画面のモデル選択の隣にあるスタイル選択メニューから切り替えます。あらかじめ用意されたプリセットと、利用者が自分で定義するカスタムスタイルの2系統で構成されています。

### プリセットスタイルから選ぶ

代表的なプリセットは、初期値のNormal（標準）、要点だけ短く返すConcise（簡潔）、背景と理由を含めて丁寧に説明するExplanatory（説明的）、ビジネス文書寄りの文体に揃えるFormal（フォーマル）の4つです。確認や事実の問い合わせはConciseで短く、仕組みの説明や社内共有資料の下書きはExplanatoryで丁寧に、社外向けメールや提案書の下書きはFormalで整える、というのが基本の使い分けです。プリセットの並びや名称は更新される可能性があるため、最新の一覧はClaude.aiの設定画面で確認してください。
...
- 外部情報や外部システムへの接続経路は、Web検索・Research・コネクタ・リモートMCPの4系統が用途別に並ぶ
