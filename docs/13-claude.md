...
本章は、Claude固有の機能と、2026年4月時点で利用時に確認したい論点、Claude側に寄せたほうが進めやすい場面の3点を扱います。取り上げる機能は、Projects、Artifacts、画像生成、コネクタ、Web検索、Research、リモートMCPサーバ、Claude Cowork、メモリ、スタイルの10個です。[8章](08-common-capabilities.md)で示した「チャット／アーティファクト／コネクタ」の共通骨格を、Claude側で具体化したものです。
...
[1章](01-gemini-in-workspace.md)ではGeminiを取っ掛かりにハンズオンを進めました。Claudeをまだ使っていない場合は、本章の機能解説へ入る前に、同じ要領で短い会話を数回試しておきます。
...
Claudeには、同じモデルを呼び出すための製品が4つあります。利用頻度が高いのはブラウザ版で、残る3つ（Cowork／Computer Use／Claude Code）は自分のPC上で動きます。本章の本題（ブラウザ版の機能）へ進む前に、4つの違いを並べます。
...
- ナレッジは絞り込む。参照頻度の高い数本に限定したほうが、回答のばらつきを抑えられる
...
リンクを開いた時点で動くデモを参照できる共有形態は、文章中心の説明より相手側の理解につながりやすい場面があります。提案、社内勉強会、採用説明など、相手に動作を見せて反応を得る局面が、Artifactsを適用しやすい用途として挙げられます。

Artifactsにはコード実行によるデータ分析の機能も含まれます。CSVファイルを添付して集計やグラフ作成を依頼すると、ClaudeがPythonコードを内部で生成・実行し、計算結果やグラフ画像をArtifacts上に返します。利用者がコードを書く必要はありません。テキストだけで数字を処理させる場合と比べ、コードが計算を担う分だけ集計の正確さが上がりやすくなります。この能力の共通的な扱いは[8章](08-common-capabilities.md)の「コード実行によるデータ分析」で取り上げています（最終確認：2026-05-27）。
...
- 利用可否はプランにより異なる。最新の対応状況は参考欄の公式ヘルプを参照する
...
仕組みそのものは4章で扱った[ツール呼び出し](04-external-system-integration.md)に相当し、ClaudeはMCP（Model Context Protocol）経由でこれらを参照しています。利用者の操作はコネクタを有効化するだけですが、裏側では4章のツール呼び出しの流れがそのまま動きます。出力が想定と異なるときの切り分けは、4章の流れをたどると進みます。
...
Web検索が会話の流れの中でその都度情報を補う仕組みであるのに対し、**Research**はテーマを渡すとClaude側が数分から数十分をかけて自律的にWebを探索し、引用元つきの構造化されたレポートを返す機能です。[11章](11-gemini-advanced.md)で扱ったGeminiのDeep Researchと同じ役割に重なります。
...
Google Workspaceアカウントを接続すると、ResearchはWeb上の公開情報に加えて、Gmail・Googleカレンダー・Googleドキュメントの内容も調査素材として取り込みます。たとえば「直近1か月の社内メールとドキュメントから、プロジェクトXの進捗を整理して」といった依頼に対して、公開情報と社内素材の双方を組み合わせたレポートを生成できます。
...
Web検索は日常のチャットに組み込む軽量な情報補完として、Researchは調査テーマに対する本格的なレポート生成として、それぞれの用途が分かれています。
...
[11章](11-gemini-advanced.md)で扱ったGemini側が保存済み情報とGemsの2系統で構成されるのに対し、Claudeは本章のProjects・メモリ・スタイルの3系統に分かれます。本節のメモリは、そのうち個人向けの1系統を指します。
...
役割分担は次のとおりです。Projectsは「この案件ではこの前提で進めてほしい」という仕事単位の文脈を、利用者が明示的に組み立てるときに使います。メモリは「日本語で答えてほしい」「要約は箇条書き形式が好み」のような、案件を問わず適用される個人の文脈に向きます。
...
- 過去の会話から得た文脈（進行中のプロジェクトの前提、以前の指示の方針ほか）

利用者から「これを覚えておいて」と明示的に指示できるだけでなく、会話の流れからClaudeが自動的に保存する場合もあります。自動保存された内容は設定画面から確認できます。
...
代表的なプリセットは、初期値のNormal（標準）、要点だけ短く返すConcise（簡潔）、背景と理由を含めて丁寧に説明するExplanatory（説明的）、ビジネス文書寄りの文体に揃えるFormal（フォーマル）の4つです。確認や事実の問い合わせはConciseで短く、仕組みの説明や社内共有資料の下書きはExplanatoryで丁寧に、社外向けメールや提案書の下書きはFormalで整える、という割り当てが基本になります。プリセットの並びや名称は更新される可能性があるため、最新の一覧はClaude.aiの設定画面で確認してください。
...
[8章](08-common-capabilities.md)では、両サービスに共通する軽量・標準・重量級の3段構造を扱いました。Claudeは現時点で、Opus／Sonnet／Haikuの3系統のモデルが並びます。重い推論はOpus、日常的な利用はSonnet、軽量で件数の多い処理はHaiku、というおおまかな住み分けです。2026年5月には **Opus 4.8** が一般提供（GA）となり、Opus系統の現行版です（最終確認：2026-06-11）。さらに2026年6月には、Opusの上位ティアとして **Mythosクラス**（一般提供版はFable 5）が加わりました。3系統の構成の上にもう1段重なる形で、利用料金は上位ほど上がる設計のため、用途と費用の釣り合いから選びます。Projectsやコネクタの裏側でも、状況に応じてモデルが切り替わる場面もあります。同じ質問への回答の傾向が以前と異なる場合は、モデル選択の指定欄を確認してください。
...
- 外部情報・外部システムへの接続経路は、Web検索・Research・コネクタ・リモートMCPの4系統に分かれ、用途別に選び取る
