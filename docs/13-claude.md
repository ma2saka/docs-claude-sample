...
本章は、Claude固有の機能と、2026年4月時点で利用時に確認したい論点、Claude側に寄せたほうが進めやすい場面の3点を扱います。取り上げる機能は、Projects、Artifacts、画像生成、コネクタ、Web検索、Research、リモートMCPサーバ、Claude Cowork、メモリ、スタイルの10個です。[8章](08-common-capabilities.md)で整理した「チャット／アーティファクト／コネクタ」の共通骨格を、Claude側で具体化したものに当たります。
...
代表例は、週次レポートの下書き・英文メールの推敲・議事録の整形といった反復のある作業です。Projectsを軸にすると、毎回同じ前提や口調をチャットの冒頭で貼り直さずに済みます。製品仕様や社内用語集、過去の意思決定ログを参照しながら進める「共通前提が重い領域」も同じ理由で向きます。小規模なチーム共有では、同僚と同じ前提で下書きやナレッジ整理を進めるための起点として使えます。
...
ナレッジの内容は、4章で扱った[ツール呼び出し](04-external-system-integration.md)とは別経路で、毎回モデルのコンテキストに添えて渡されます。詰め込みすぎるとコンテキストウィンドウを圧迫し、回答品質に影響します（仕組みの背景は[7章](07-terminology.md)）。参照させたい資料が数十本以上の規模になると、手動の管理が難しくなります。その場合はRAG（検索拡張生成）と呼ばれる仕組みが選択肢に入ります。RAGの概念と用語の整理は[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で扱っています。
...
Artifactsにはコード実行によるデータ分析の機能も含まれます。CSVファイルを添付して集計やグラフ作成を依頼すると、ClaudeがPythonコードを内部で生成・実行し、計算結果やグラフ画像をArtifacts上に返します。利用者がコードを書く必要はありません。テキストだけで数字を処理させる場合と比べ、コードが計算を担う分だけ集計の正確さが上がりやすくなります。この能力の共通的な位置づけは[8章](08-common-capabilities.md)の「コード実行によるデータ分析」で整理しています（最終確認：2026-05-27）。
...
- 生成した画像を社外向けの制作物に使う場合は、AI生成物としての表示義務や商用利用の可否を利用規約で確認する。権利と表示の考え方は[3章（マルチモーダル）](03-multimodal.md)で整理している
...
「Workspaceの内側はGeminiが順当、Workspaceの外側や横断利用はClaude」という[8章](08-common-capabilities.md)の整理が、この並びにそのまま現れます。SlackやHubSpot、Zoomなど、Workspaceに隣接して使われるSaaSが標準コネクタに揃うのがClaude側の特徴です。
...
**メモリ**（Memory）は、利用者の好み・作業パターン・事実情報などを、会話をまたいで長期的に保持する仕組みです。[5章](05-misunderstanding-learning.md)で整理した「コンテキストとメモリ」のうち、メモリの実際の使い方を本節で扱います。
...
[8章](08-common-capabilities.md)では、両サービスに共通する軽量・標準・重量級の3段構造を整理しました。Claudeは現時点で、Opus／Sonnet／Haikuの3系統のモデルが並びます。重い推論はOpus、日常的な利用はSonnet、軽量で件数の多い処理はHaiku、というおおまかな住み分けです。2026年5月には **Opus 4.8** が一般提供（GA）となり、Opus系統の現行版です（最終確認：2026-06-11）。さらに2026年6月には、Opusの上位ティアとして **Mythosクラス**（一般提供版はFable 5）が加わりました。3系統の構成の上にもう1段重なる位置づけで、利用料金は上位ほど上がる設計のため、用途と費用の釣り合いから選びます。Projectsやコネクタの裏側でも、状況に応じてモデルが切り替わる場面もあります。同じ質問への回答の傾向が以前と異なる場合は、モデル選択の指定欄を確認してください。

Mythosクラスには、データの扱いで1点だけ特記があります。安全確保の目的で、入出力は提供経路を問わず30日間保持される方針が示されており、保持なし（ゼロデータ保持）の構成では利用できません（最終確認：2026-06-11）。[9章](09-security-individual.md)で整理した「サービス提供者側のログ」の枠に重なる話で、組織のデータ取扱の要件によっては、Mythosクラスだけ利用可否の判断が分かれる場合もあります。組織で利用する場合は、契約・データ取扱の担当への確認が要点になります。
...
なお、ここまで本章で扱ってきたのは`https://claude.ai`の個人・組織プランですが、組織契約では同じモデル群をAWS・Google Cloud・Microsoftのクラウド経由で利用する形もあります。経路ごとに契約の相手方やデータを処理する主体が変わります。違いの整理は[10章](10-security-agent-era.md)の契約経路の節を参照してください。
