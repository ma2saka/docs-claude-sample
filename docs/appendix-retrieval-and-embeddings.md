...
本ドキュメントの他章で具体名を挙げているサービスでは、[12章のNotebookLM節](12-google-workspace-and-gemini.md)が「投入資料の範囲内で回答する」設計として、上表の3〜4行目に近い動き方をします。[13章のProjects節](13-claude.md)で扱う「ナレッジ」は、資料を毎回コンテキストへ添える経路であり、表に並べたRAGの段取りには踏み込まない軽量な作りです。参照させる資料が数十本以上に増えると、同章でも本付録のRAG構成が選択肢として案内されています。
