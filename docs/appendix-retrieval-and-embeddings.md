...
利用者の感覚としては、「グラウンディングはRAGの言い換えのようなもの」と捉えても大きく外れません。設計者目線では、Geminiのグラウンディング機能を使うときに運用面の縛りが追加で乗ってきます。Google検索によるグラウンディングは検索候補の表示が利用条件になっており、こうした実務上の差が出てくる点は押さえておきたいところです。
