...
- **メモリ機能** — ユーザーに関するメモを長期間保存し、次の会話でも参照できるようにする仕組み（ChatGPTの「メモリ」、Geminiの「Gems」等のユーザー情報、Claudeの「Projects」の知識ベースなど）
...
```text
昨日の発話：「私はマーケティング職で……英語B2B向け」
 ↓ サービス内のメモ領域（メモリ/プロジェクト知識/会話履歴）に保存
 ↓
今日の発話：「前に言ったトーンで書いて」
 ↓ 裏で、保存していた昨日のメモを添えてモデルに渡す
 ↓
モデル：英語B2Bトーンで応答
```
