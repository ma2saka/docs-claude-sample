...
本章では、[9章](09-security-individual.md)で整理した個人利用の論点（入力・出力・履歴）に積み増す形で、組織のルールが敷かれる背景と、エージェントの自走や複数サービス連携の場面で追加になる論点を扱います。追加の論点は、サンドボックス・MCP／コネクタの境界・確認をどこに絞るか、の3つです。
...
なお、本章は契約済みのクラウド型サービスを前提に組織ルールの背景を整理しています。提供事業者のクラウドそのものを経由しない経路（自社サーバや手元のPCでオープンウェイトのモデルを動かす形）を業務に組み込む選択肢もあり、上の表の観点はその場合でも基本的に当てはまります。選択肢の整理と、検討時に変わる論点は[Appendix: ローカルLLM](appendix-local-llm.md)で扱います。
...
[9章](09-security-individual.md)で整理した3つの観点（入力・出力・履歴）は、利用者が毎回キーボードに向かって入力することを暗黙の前提にしていました。エージェントが自走する場面では、この前提が次のように崩れます。
...
ZapierやMakeのようにSaaS同士をハブで結ぶワークフローツールでも、1つの自動化フローに複数の権限が集まる構造は同じです。生成AIの差し込み方と、権限の置き方は[Appendix: ワークフローツール](appendix-workflow-tools.md)で整理しています。
...
3点とも、特別な道具は必要ありません。[9章](09-security-individual.md)で整理した3つの観点に、人が間に入らない経路を想定した注意を一段足しただけの内容です。
