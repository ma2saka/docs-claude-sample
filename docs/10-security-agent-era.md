...
[Appendix: Claude Code](appendix-claude-code.md)で触れた「小さなフォルダで、人の確認を経由しながら、徐々に広げる」という進め方は、サンドボックスの具体的な実践と同じ内容です。同じ枠組みは、自分のPC上の手作業を生成AIに任せる場面でもそのまま当てはまります。3つのアプローチ（ランチャー型・画面操作エージェント型・スクリプト生成型）の向き不向きと、誤操作の波及を抑える対策は[Appendix: デスクトップの自動化](appendix-desktop-automation.md)で扱っています。
