...
なお、Cloud Next 2026では、Sheets上に対話的なダッシュボード・カンバン・ヒートマップなどを配置できるSheets canvas（mini-apps）の追加も発表されました。表データの上に簡易のアプリ的なビューを重ね、Geminiとの対話で可視化を調整する用途で、本ドキュメント執筆時点ではロールアウト中です（最終確認：2026-05-16）。
...
本ドキュメント執筆時点ではロールアウト中で、プランやアカウントによって到達順が分かれます。手元のGmailで該当機能が見当たらない場合は、未到達である可能性を最初に確認してください（最終確認：2026-05-16）。
...
### Workspace Studio: no-codeのエージェント作成ツール（ロールアウト中）
...
組織側からは、作成されたエージェントを中央のレジストリで一覧・監査できます。本ドキュメント執筆時点ではロールアウト中で、利用可否はWorkspaceプランによります（最終確認：2026-05-16）。
