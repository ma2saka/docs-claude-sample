...
Googleドキュメントのサイドパネルは、本章で扱う5つのアプリの中で利用頻度が高い部類に入ります。依頼の粒度は、まっさらからの下書き／既存文章の変換／構成の整理の3段に分かれます。
...
Workspaceアプリ内のサイドパネルだけがGeminiの入口ではありません。同じGoogleアカウントから利用できる関連サービスや、Workspace内の自動化を組み立てる仕組みに、用途の異なるAIツールが用意されています。本ドキュメントで取り上げるのは次の3つです。
...
いずれも、目に見える事故というよりは、気づきにくい形で作業効率を落とすタイプのつまずきです。本節で並べた5項目は、利用前のチェック項目としてそのまま参照できる粒度に揃えてあります。
