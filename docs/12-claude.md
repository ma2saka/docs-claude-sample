...
Claude Code（自分のPCで動くClaude）については、本書では[Appendix: Claude Code](appendix-claude-code.md)で別立てに扱います。本章はあくまでブラウザの`https://claude.ai`とデスクトップアプリから始める使いこなしを中心に据えます。

## 対象読者と前提

- [7章](07-common-capabilities.md)の3本柱（チャット・アーティファクト・コネクタ）に目を通した人
...
- 役割を1つに絞る。「なんでもできるProject」は、結局なにも得意でないProjectになりがち
- ナレッジは盛りすぎない。資料を全部突っ込むより、**よく参照する数本**に絞ったほうが回答の安定度は上がる
- 共有権限は最小から。`Can view`から始めて、必要になってから`Can edit`へ広げる順が事故を呼びにくい

ナレッジの中身は3章で扱った[ツール呼び出し](03-external-system-integration.md)とは別経路で、**毎回モデルのコンテキストに添えられる**形で渡されます。詰め込みすぎるとコンテキストウィンドウを圧迫し、回答品質を落としかねない点だけは覚えておいてください（仕組みの背景は[6章](06-terminology.md)）。

## Artifacts: チャットの隣に成果物を置く

[7章](07-common-capabilities.md)でアーティファクトの概念を扱いました。Claudeにおけるアーティファクトが**Artifacts**です。チャット画面の右側に作業ペインが開き、文書・コード・HTML・図などを「対話しながら磨く」ことができます。
...
- **公開リンクは「公開」** — 共有リンクは原則「URLを知っている人」なら誰でも開ける。社外秘の素材を貼ったまま公開のチェックを入れたままにしない（[8章](08-security-individual.md)）
- **本番代わりにしない** — 動くプレビューはあくまで試作の場。普段使いに乗せるなら、別途ホスティングを用意してそちらへ移す
- **バージョン管理は軽量** — Artifactsにも履歴は残るが、Gitほど堅牢ではない。重要な版は手元にも保存しておく

「読者がリンクを踏めば、即座に動くデモが立ち上がる」という体験は、文章中心の説明より圧倒的に伝わります。提案・社内勉強会・採用説明など、**人に見せて反応をもらう局面**でArtifactsの真価が出ます。

## コネクタ: 外のサービスを直接呼ぶ

7章で扱ったコネクタの、Claude側の顔ぶれを並べます。2026年4月時点で、`https://claude.ai`から有効化できる代表的なものは次のとおりです。

| カテゴリ | 例 | できること（ざっくり） |
| ---- | ---- | ---- |
| ビジネスチャット | Slack | 指定チャンネルやスレッドの検索、メンションのキャッチアップ |
| メール／ドライブ／カレンダー | Gmail、Googleドライブ、Googleカレンダー | スレッド要約、関連資料の参照、空き枠の把握 |
| 会議 | Zoom | 録画／文字起こしを材料にした要約・タスク抽出 |
| CRM | HubSpot | 取引先・案件・活動履歴の検索と整形 |
| ナレッジ／タスク | Notion、Linear、Jira | ページ／チケットの参照、横断検索 |

「Workspaceの内側はGeminiが近道、Workspaceの外側や横断利用はClaudeが強い」という7章の整理は、この表でも裏が取れます。SlackやHubSpot、Zoomといった**Workspaceの隣にあるSaaS**まで素直に並ぶのが、Claudeのコネクタ周辺の手ざわりです。

仕組み自体は3章で扱った[ツール呼び出し](03-external-system-integration.md)そのものであり、Claudeは内部的にMCP（Model Context Protocol）経由でこれらを呼んでいます。読者の感覚としては「ボタンを押して有効化するだけ」ですが、裏では3章の流れがそのまま走っている、という補助線を引いておくと、出力がおかしいときの当たりが付けやすくなります。

### コネクタを有効にするときの最小チェック

- **読み取りで足りるなら書き込みを渡さない** — 用途が読み取りで完結するのに書き込み権限を渡してしまう、が一番事故りやすい
- **個人アカウントと業務アカウントを混ぜない** — 業務データを個人アカウント経由のClaudeに通してしまう事故は、いまだに上位常連
- **使わなくなったら外す** — 連携を残したままにすると、コンテキストへ入り得る情報の総量だけ静かに増える

詳細は7章のチェックリストと、個人利用視点の[8章](08-security-individual.md)で扱った内容がそのまま当てはまります。

## リモートMCPサーバ: 自分で繋ぎ口を増やす

公式コネクタの一覧を見ても、自社の独自ツールや社内ナレッジ基盤までは入っていない、というのが現実です。Claudeは、ここで**リモートMCPサーバ**を自分で登録できる、という抜け道を用意しています。

イメージは、家電量販店に並んでいるコネクタの隣に、**自作の差込口を持ち込んで挿せる**ようなものです。
...
10章で触れたとおり、Geminiは2026年4月時点で「任意のMCPサーバを自由に挿す」使い方が標準のチャット画面からは難しい状況です。**コネクタの自由度の高さは、現時点でのClaudeの分かりやすい強み**だと言えます。

ただし、自由度が高い道具ほど、扱いには敬意が要ります。
...
このあたりはエージェントの話と地続きなので、組織で本格的に使い始めるときは[9章](09-security-agent-era.md)を必ず通っておいてください。
...
### Computer UseやClaude Codeとの関係

似て非なる道具がいくつか並ぶ領域なので、ひと言で関係を整理しておきます。

| 道具 | 主戦場 | 動かし方 | 詳しくは |
| ---- | ---- | ---- | ---- |
| Claude Cowork | 自分のPC上の日々の作業 | デスクトップアプリの常駐エージェント | 本節 |
| Claude Computer Use | APIや連携が塞がれた画面の操作 | 画面を見てマウス・キーボードを動かす | [Appendix: デスクトップの自動化](appendix-desktop-automation.md) |
| Claude Code | 開発まわりの一連作業 | CLIやIDE拡張から呼ぶ | [Appendix: Claude Code](appendix-claude-code.md) |

非エンジニアの方が最初に触る順番としては、**ブラウザ → デスクトップアプリ（Cowork）→ Computer UseやClaude Code**という階段が穏当です。下の段から順に踏むほど、事故の規模と学習コストの両方を低く抑えられます。

## 2026年春時点で押さえておきたいこと

Claudeは半年単位で機能が増えていく道具です。2026年4月時点で、地味につまずきやすい現実をいくつか先回りで挙げておきます。
...
「個人プランでは動いた機能が会社プランで見当たらない」「逆に会社プランでだけ出てくるメニューがある」のは、アプリの故障ではなく**プラン設計の差**です。プラン名と利用可否は、参考欄の公式ヘルプで都度確認してください。

### コネクタの顔ぶれは月単位で変わる

公式コネクタは、2026年4月時点で50を超える勢いで増えています。本章の表は執筆時点のスナップショットに過ぎません。3か月後に見たら、表に並んでいるアプリの顔ぶれが違っていても驚かないでください。
...
## ClaudeとGemini、使い分けの再確認

[10章](10-gemini-advanced.md)の表と対になる形で、Claude側からの視点で並べます。

| 状況 | 向いているほう | 理由 |
| ---- | ---- | ---- |
| Slack・HubSpot・NotionなどWorkspaceの外側のSaaSを材料にしたい | Claude | コネクタの顔ぶれが厚い |
| 自社の独自ツールにMCPで繋いで自動化したい | Claude | リモートMCPの自由度が高い |
| 動くプレビュー付きの試作を作って人に見せたい | Claude | Artifactsから公開リンクを直接出せる |
| 自分のPC上の複数ファイルをまとめて任せたい | Claude（Cowork／Claude Code） | デスクトップ寄りの選択肢が揃っている |
| Docs・Sheets・Gmailの画面に居たまま手伝ってほしい | Gemini | サイドパネル統合が距離ゼロ |
| 画像・音声・動画を混ぜた素材処理 | Gemini | マルチモーダルの対応幅が広い |

この表も執筆時点の傾向であり、半年も経てば配置が動いている可能性は十分にあります。「とりあえず使い始めて、壁にぶつかったら隣のサービスを覗く」くらいの軽さで構えるのが、結局いちばん速い学び方です。
