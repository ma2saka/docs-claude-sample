...
## Claudeファミリー俯瞰: ブラウザ版と、PC寄りの3兄弟

Claudeブランドには、同じ頭脳を別の入口で使うための派生が並びます。非エンジニアの読者が最初に迷いやすいのは、ブラウザ版と**PC寄りの3兄弟**（Cowork／Computer Use／Claude Code）の違いです。本章の本題（ブラウザ版の使いこなし）へ進む前に、4つの顔ぶれを横並びで押さえておきます。

| 道具 | 主戦場 | 動かし方 | 人の確認ポイント | 詳しくは |
| ---- | ---- | ---- | ---- | ---- |
...
最初に触る階段は上から下です。ブラウザ版でClaudeの手触りに慣れたら、CoworkでPC側へ広げ、必要があればClaude Codeで開発寄りの仕事へ進みます。Computer Useは**他の道が塞がれたときの最後の選択肢**として温存します。

```mermaid
flowchart TB
    B[ブラウザ版 Claude<br/>会話・アーティファクト] --> C[Claude Cowork<br/>デスクトップ常駐]
    C --> K[Claude Code<br/>CLI・IDE]
    C -.最後の手段.-> U[Claude Computer Use<br/>画面直接操作]
```

4つの入口は**同じClaudeという頭脳を別の器で動かすもの**で、頭脳そのものの違いではありません。上から下へ順に踏んでいくと、学習コストも事故の影響範囲も段階的に広がります。「入口の違い＝手と目をどこまで貸すか」と捉えておくと、選ぶ場面で迷いにくくなります。
...
「Workspaceの内側はGeminiが近道、Workspaceの外側や横断利用はClaude」という7章の整理は、この表でも裏が取れます。SlackやHubSpot、Zoomといった**Workspaceの隣にあるSaaS**が、標準コネクタの一覧にそのまま並ぶのが、Claude側の見え方です。
...
このあたりはエージェントの話と地続きなので、組織で本格的に使い始めるときは[9章](09-security-agent-era.md)を先に通しておくことをおすすめします。
...
Coworkと、Claude Code／Computer Use／ブラウザ版Claudeの横並びは、本章冒頭の「Claudeファミリー俯瞰」節で表と階段図にまとめました。
