...
本章では、「学習」と呼ばれがちな3つの現象を分解し、そのうえで「入力データはどう扱われるのか」を、APIと消費者向けUIとで分けて整理します。[4章](04-external-system-integration.md)末尾でも触れたとおり、ツール呼び出しと「学習」を切り分けられると、実務の判断軸を分けて立てられます。
...
ここまでの2つ（事前学習・ファインチューニング）は、業務利用の通常フローで自動的に発生することはありません。明示的に「ファインチューニングします」というメニューを選択しない限り、モデル本体は更新されません。なお、調整したモデルや既存のオープンウェイトのモデルを、提供事業者のクラウドではなく自社内や手元のPCで動かす選択肢もあります。動機・ツール・動かす環境の整理は[Appendix: ローカルLLM](appendix-local-llm.md)で扱います。

### コンテキストとメモリ

実務で「学習されてしまうのでは」として懸念されるのは、おもにこの領域です。モデル本体はまったく変わらないのに、覚えているように振る舞う現象を指します。
...
| メモリ機能／プロジェクト知識 | ユーザーや仕事に関するメモを長期間保存し、次の会話でも参照できるようにする仕組み（Claudeの「Projects」に紐づく知識ベース、Geminiの保存済み情報やGemsのカスタム指示など） |

いずれも、モデルの重みには書き込みません。書き込まれるのはモデルの外側にある記憶領域だけです。[7章（用語集）](07-terminology.md)ではコンテキストを「机の上の資料の山」にたとえています。その対応関係に沿うと、事前学習は脳そのものの発達、メモリは机へ追加する付箋に対応します。メモリ機能の具体的な使い方は、Claude側を[13章](13-claude.md)、Gemini側を[11章](11-gemini-advanced.md)に分けて取り上げます。社内文書のように規模の大きい資料群をモデルの外側に置き、毎回の会話で関連する一部だけを引いてくる経路は、RAG・埋め込み検索の段取りとして[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で扱います。
...
- Google Workspaceのビジネスプランや、各社のエンタープライズ契約を使っている場合、組織としての合意がすでに結ばれている可能性が高い。契約・利用ルールを把握している担当（IT・法務・セキュリティなど）に問い合わせる
- APIを自社プロダクトに組み込む場合は、利用規約で「入力内容は学習に使わない」旨を確認し、ログの保存期間も把握する

## 「学習される／されない」は4つの観点で判断する
...
見落としやすいのは4番目です。学習利用がオフでも、会話履歴やメモは通常どおり残ります。使い終わったら消すルールを組織の標準として決めておくと、消し忘れを防ぎやすくなります。入力・出力・履歴の3領域に分けた判断基準は、[9章（セキュリティ 個人利用編）](09-security-individual.md)で扱います。
...
表の1行目「会社の資料をアップしたら学習される？」が念頭に置いている『資料を参照させる仕組み』そのものは、事前学習やファインチューニングとは別軸の経路です。社内文書を会話のコンテキストに合流させる段取り（RAG・埋め込み検索・グラウンディング）は[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で整理しています。

## まとめ

- 「学習」には、事前学習・ファインチューニング・コンテキスト／メモリの3種類が含まれる。まずはこれを分けて話す
- 普段の利用で気になるのは、ほとんどがコンテキストやメモリの話である。モデル本体は変わっていない
