...
1行目の「会社の資料をアップしたら学習される？」が念頭に置いている「資料を参照させる仕組み」そのものは、事前学習やファインチューニングとは別軸の経路にあたります。社内文書を会話のコンテキストに合流させる段取り（RAG・埋め込み検索・グラウンディング）は[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で整理しています。
