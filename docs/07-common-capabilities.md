...
最後の行のように、最近はテキスト以外の入力もチャットの延長で扱えます。**マルチモーダル**と呼ばれる領域で、Claude・Geminiの双方が画像・PDF・音声の読み取りを標準でサポートしています（動画は対応が広がりつつある段階です）。たとえばスクリーンショットを貼って「ここの項目を表にして」と頼むような使い方は、手軽に効きやすい一例です。はまる場面は業務内容によって違うので、まずは手元の仕事で小さく試すのがおすすめです。
