...
Claudeにも、Geminiにも、代表的な外部サービスとのコネクタが用意されています。本ドキュメントでは、**両社の対応コネクタと使い分けの感覚を本章にまとめ**、10章・12章では個別サービス側の深掘りに集中します。
...
この早見表は方向感を揃えるための地図に過ぎません。実務では、使いながら自分の手仕事に合わせて並びを更新していくのが自然です。半年後には配置が動いているのが普通、くらいの心構えで眺めてください。Gemini側の具体像は[10章](10-gemini-advanced.md)、Claude側の具体像は[12章](12-claude.md)で扱います。
...
この仕組みがわかっていると、セキュリティの感覚も自然に身につきます。「コネクタを1つ繋ぐ」とは、**その範囲の情報がAIのコンテキストに入る可能性を開くこと**です。組織のルールと利用者の分担を観点ごとに並べる話は8章、エージェントまで踏み込んだ話は9章で扱います。
