...
本章は、生成AIに画像・音声・動画・PDFといったテキスト以外の素材を渡すとき、内側で何が起きているかを2章で押さえた仕組みの延長として整理します。マルチモーダル対応のチャット画面で扱える入力素材と、画像生成や音声合成といった出力の現状を眺め、業務での組み合わせ方と、本ドキュメントで深追いしない領域を確認します。各社の最新仕様や個別プロダクトの使いこなしは、後続章と一次ソースに委ねます。
...
仕組みは2章でなぞった枠組みの延長として位置づけられます。画像ならピクセルの並び、音声なら音の波形を、それぞれモデルが処理できるベクトル表現に変換する前段が用意されており、その先の生成処理は文章のときと近い形で動きます。前段の実装はモデルごとに異なり、画像や音声を扱う場合は専用のエンコーダ層やembedding空間への射影といった、テキストとは別の処理を組み合わせるのが通例です。
...
入口と出口の様式は別物に見えますが、いったんモデルが扱える表現にそろえてしまえば、文章生成のときと近い枠組みでまとめて扱えます。新しいプロダクトであっても、入口にこの前段が同じ役割で置かれているかぎり、2章で扱った像の延長として位置づけられます。
...
本ドキュメントで押さえたいのは、入力と出力の様式が広がっても、根の仕組みは2章のテキスト生成と変わらない、という像です。この前提が共有されていれば、新しいプロダクトを目にしたときも、自分の業務に取り込んでよい範囲を、テキスト生成のときと同じ枠組みで判断できます。
