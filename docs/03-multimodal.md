...
入口と出口の様式は別物に見えますが、いったんモデルが扱える表現にそろえてしまえば、文章生成のときと近い枠組みでまとめて扱えます。新しいプロダクトであっても、入口にこの前段が同じ役割で置かれているかぎり、[2章](02-what-is-generative-ai.md)で扱った像の延長として位置づけられます。
...
ここで整理した素材を、生成AIの主要機能（チャット・アーティファクト・コネクタの3本柱）の中でどう扱うかは、[8章（生成AIでできること 共通編）](08-common-capabilities.md)で整理しています。スクリーンショットの読み取りやPDFの要約は、同章のチャット節で扱う「テキスト以外の入力もチャットの延長で扱える」場面の代表例にあたります。
...
実務でマルチモーダルらしさが見える場面は、1回の依頼に複数の素材を同時に渡すときです。画像とテキスト、PDFと表、音声と文章のような束ね方をすれば、それまで人手でつなぎ合わせていた工程を1回の依頼にまとめられます。
...
本ドキュメントで押さえたいのは、入力と出力の様式が広がっても、根の仕組みは[2章](02-what-is-generative-ai.md)のテキスト生成と変わらない、という像です。この前提が共有されていれば、新しいプロダクトを目にしたときも、自分の業務に取り込んでよい範囲を、テキスト生成のときと同じ枠組みで判断できます。
