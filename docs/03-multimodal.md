...
- **「画像のままで読み取れる」のが地味に効く** — スクリーンショットを貼って「ここを表にして」と頼む手順は短いわりに、効きが大きい用途。OCRを別途回す手間を考えれば、まず試してみる価値がある
- **PDFは「全部投げる」より「要点だけ抜く」が安定** — コンテキストの上限を超えると、巨大なPDFは末尾から落ちることがある。章節を絞って渡す、見出しだけ先に渡す、といった段取りで近づく
...
| 種類 | 何が得られるか | 代表的な入口の例 |
| ---- | ---- | ---- |
| 画像生成 | テキスト指示から静止画を生成、生成済み画像の編集 | Geminiアプリ（Imagen系）、ChatGPT、Claudeなど各社 |
| 動画生成 | テキストや参照画像から短い動画クリップ | Geminiアプリ（Veo系）、ChatGPTのSora系、専用ツール |
| 音声合成 | 文章を音声にする、声質を選ぶ／指定する | 各社のテキスト読み上げ、専用サービス |
| 音声対話 | こちらの発話を聞き取り、音声で応答する | ChatGPTの音声モード、Geminiの音声対話、Claudeの音声入出力 |
