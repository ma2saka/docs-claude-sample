...
仕組みの骨格は2章でなぞった次トークン予測の延長にあります。テキスト以外の素材は、それぞれ専用の前段（画像エンコーダ、音声エンコーダなど）を通じて、モデルが扱えるベクトル表現に変換されます。テキストの埋め込みと同じモデル内部に並べて扱える形に揃えたうえで、後段の確率計算（2章で扱った仕組み）が、トークンを1つずつ生成します。前段の処理は様式ごとに違いがあり、テキストのトークン化と同一ではありませんが、後段の計算はテキストと共通です。

```mermaid
flowchart LR
    T["文章"] --> E["モデル内部の<br/>ベクトル表現"]
    I["画像"] -->|画像エンコーダ| E
    A["音声"] -->|音声エンコーダ| E
    V["動画"] -->|動画エンコーダ| E
    E --> M["モデル<br/>（後段の確率計算）"]
...
入口と出口の様式は別物に見えますが、前段が表現をモデルの扱える形に揃え、後段は2章で押さえた確率計算によって応答を組み立てる、という構図は共通です。新しいプロダクトを目にしたときも、前段（様式ごとの変換）と後段（テキストと共通の確率計算）の2段で構成されていると捉えると、2章で押さえた像の延長として整理できます。
