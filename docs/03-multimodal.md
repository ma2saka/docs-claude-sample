...
本章は、生成AIに画像・音声・動画・PDFといったテキスト以外の素材を渡すとき、また画像や音声を生成させるときに、内側で何が起きているのかを整理します。入力側はLLMの拡張として2章の仕組みの延長で説明できる一方、出力側は別系統のモデルを組み合わせる構成が多くなります。マルチモーダル対応のチャット画面で扱える入力素材と、画像生成や音声合成といった出力の現状を眺め、業務での組み合わせ方と、本書では深追いしない領域を確認します。各社の最新仕様や個別プロダクトの使いこなしは、後続章と一次ソースに委ねます。
...
## マルチモーダルは、複数の様式の情報をひとつのモデルで束ねて扱う性質
...
入力側の処理は2章でなぞったとおりです。画像はビジョンエンコーダ、音声は音声エンコーダといった専用のニューラルネットワークを通り、文章のトークンと並べて使えるベクトル列に変換されてからLLMに渡されます。LLM側は、文章のトークンとこれらのベクトルをひとつの入力列として読み、次のトークンの確率分布を出して応答（多くはテキスト）を組み立てます。

出力側で画像や音声を生成するときは、テキスト生成と同じモデルを使うのではなく、画像なら拡散モデル、音声なら音声合成モデルといった別系統のモデルを呼び出す構成が一般的です。チャット画面の上では1つの会話に見えても、内側では入力と出力で違う種類のモデルが組み合わさっている、と捉えるほうが、各社の機能差を読み解きやすくなります。

```mermaid
flowchart LR
    T["文章"] --> ET["テキスト<br/>トークン化"]
    I["画像"] --> EI["ビジョン<br/>エンコーダ"]
    A["音声"] --> EA["音声<br/>エンコーダ"]
    V["動画"] --> EV["フレーム＋<br/>音声エンコーダ"]
    ET --> M["LLM<br/>（応答を組み立て）"]
    EI --> M
    EA --> M
    EV --> M
    M --> OT["文章の出力"]
    M -.別モデル呼び出し.-> OI["画像の出力<br/>（拡散モデル等）"]
    M -.別モデル呼び出し.-> OA["音声の出力<br/>（音声合成モデル）"]
```

新しいプロダクトを目にしたときも、入力側がLLMにベクトルを束ねて渡しているのか、出力側で別系統のモデルが呼ばれているのかを見ると、2章で押さえた像の延長として位置づけられます。
...
業務でマルチモーダルを使い始めると、より細かな仕様や運用への踏み込みが必要になる場合があります。本書では、次のような領域には深追いせず、別の場所に委ねる立場をとります。
...
- マルチモーダルは、入力側でテキスト以外の素材を専用エンコーダでベクトルに変換し、LLMにテキストと並べて渡す形で実現している
- 出力側で画像や音声を生成するときは、テキスト生成とは別系統のモデル（拡散モデル・音声合成モデルなど）を呼び出す構成になっていることが多い
