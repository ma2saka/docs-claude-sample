...
「いきなりエージェント的自走から始める」分岐をフローに含めていないのは意図したものです。自走型は、ノード型と司令塔型で使い方の作法を整えてから順番に追加する形のほうが、副作用を制御しやすくなります。なお、自動化の対象がWorkspaceアプリ内で完結する場合は、前節で触れたWorkspace Studioも選択肢に入ります。
