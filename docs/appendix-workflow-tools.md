...
n8nはOSSで、自社サーバ上で動かせます。データを社外へ出す経路を最小化したい、監査ログを自前で保持したい、といった要件があるときの選択肢です。最近はLLMノードやベクトル検索ノードもそろっており、エンジニアリング体制があれば、ハブ型で実現していることはおおむね再現できます。

### LLMアプリ志向型

Difyは目的が異なります。「LLMを中心に据えたミニアプリをノーコードで作り、そのアプリをAPI経由で外部から呼び出す」という発想です。プロンプト管理、ナレッジベース、評価機能など、生成AIを活用するうえで必要な機能が同居しており、チャットボットやRAGアプリを継続的に作る現場に向きます。
...
Google Workspaceを日常の業務環境としている読者には、上記3タイプとは別に、Workspace内で完結する自動化の選択肢があります。Cloud Next 2026で発表された**Workspace Studio**は、Gmail・Docs・Sheets・Drive・Meet・Chatをまたぐエージェントをno-codeで作成する基盤です。Asana・Jira・Salesforceなど外部SaaSとの接続も備えており、Workspace内の素材を起点にした定型業務の自動化に向きます。組織側からは、作成されたエージェントを中央のレジストリで一覧・監査できる仕組みも用意されています。
...
本ドキュメント執筆時点ではロールアウト中で、利用可否はWorkspaceプランによります。セットアップの詳細は[12章のWorkspace Studio節](12-google-workspace-and-gemini.md)を参照してください（最終確認：2026-05-16）。
