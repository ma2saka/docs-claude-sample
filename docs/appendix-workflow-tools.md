...
この型は強力ですが、プロンプトは雪だるま式に肥大し、再現性も落ちやすくなります。プロンプトをコードと同じようにバージョン管理し、テストケースで挙動を固定する運用が現実的です。
...
「いきなりエージェント的自走から始める」分岐がないのは、**意図的に外しています**。自走型は、ノード型と司令塔型で運用の土台ができてから足すものだと考えてください。最初からハーレーで通勤を始める人がいないのと同じ理屈です。
...
いきなり完璧な自動化を狙わず、**たたき台を人が校正する運用**で3週間ほど回してみるのが、結局いちばん速い道筋です。
