...
- 自分のPC上で完結する作業や、手元のCLIから生成AIに依頼する経路と読み比べたい人（隣接する[Appendix: デスクトップの自動化](appendix-desktop-automation.md)、[Appendix: Claude Code](appendix-claude-code.md)）
...
ZapierとMakeは長い実績のあるサービスで、既製のコネクタ（組み込み済みの接続先）が数千種類用意されています。生成AIのノードも標準で備わっており、「届いたメールをClaudeに要約させてSlackに流す」ような構成を、画面操作だけで組み立てられます。導入の手数が少なく、設定項目の説明も画面上に表示されます。商用SaaSのため、利用が増えると課金額もそのまま増えます。

### OSS／セルフホスト型

n8nはOSSで、自社サーバ上で動かせます。データを社外へ出す経路を最小化したい、監査ログを自前で保持したい、といった要件があるときの選択肢です。最近はLLMノードやベクトル検索ノードもそろっており、エンジニアリング体制があれば、ハブ型で実現していることはおおむね再現できます。LLMノードから呼び出すモデル自体も、提供事業者のクラウドではなく自社サーバ上で動かす構成が選べます。選択肢の整理は[Appendix: ローカルLLM](appendix-local-llm.md)で扱っています。

### LLMアプリ志向型

Difyは目的が異なります。「LLMを中心に据えたミニアプリをノーコードで作り、そのアプリをAPI経由で外部から呼び出す」という発想です。プロンプト管理、ナレッジベース、評価機能など、生成AIを活用するうえで必要な機能が同居しており、チャットボットやRAGアプリを継続的に作る現場に向きます。RAG（検索拡張生成）やベクトル検索の仕組みと用語は[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で整理しています。

3タイプの境界は明確に分かれているわけではなく、重なる領域もあります。実務では「SaaS同士の接続はハブ型、生成AI側の処理はLLMアプリ志向型、それらをOSSのワークフローで束ねる」という組み合わせも珍しくありません。

### Google Workspace を業務環境とする場合の補足: Workspace Studio

Google Workspaceを日常の業務環境としている場合は、上記3タイプとは別に、Workspace内で完結する自動化の選択肢があります。Cloud Next 2026で発表された**Workspace Studio**は、Gmail・Docs・Sheets・Drive・Meet・Chatをまたぐエージェントをno-codeで作成する基盤です。Asana・Jira・Salesforceなど外部SaaSとの接続も備えており、Workspace内の素材を起点にした定型業務の自動化に向きます。組織側からは、作成されたエージェントを中央のレジストリで一覧・監査できる仕組みも用意されています。
...
ワークフローの中で生成AIを使う形は、3つのパターンに分けられます。それぞれ向いている用途と、想定すべきリスクが違うため、名前で区別しておくと検討の場で論点をそろえやすくなります。
...
この型は表現力が高い一方で、プロンプトが次第に大きくなり、再現性も落ちやすくなります。プロンプトをコードと同様にバージョン管理し、テストケースで挙動を確認する進め方が、長期にわたって扱いやすい形になります。

### エージェント的自走

[7章](07-terminology.md)で紹介した「エージェント」を、ワークフローの一部として組み込む型です。ゴールだけを与え、使えるツールの一覧を渡して、その先の手順はLLMに任せます。うまく回る場面では成果が大きい一方、想定外のツールを想定外の順で呼ぶ副作用が伴います。

使い始めの段階では、実行を人の手前で止めて承認する段取りを残します。[10章](10-security-agent-era.md)で扱うサンドボックス環境と操作ログの確保は、この型を使い続けるうえでの前提として位置づけます。
...
「いきなりエージェント的自走から始める」分岐をフローに含めていないのは意図したものです。自走型は、ノード型と司令塔型で使い方の作法を整えてから順番に追加する形のほうが、副作用を制御しやすくなります。なお、自動化の対象がWorkspaceアプリ内で完結する場合は、前節で触れたWorkspace Studioも選択肢に入ります。自動化の対象が自分のPC上のアプリ操作・ファイル整理に寄る場合は、本付録の3タイプの外側にも選択肢があります。画面操作型の自動化は[Appendix: デスクトップの自動化](appendix-desktop-automation.md)で扱います。自然言語の指示でローカルのファイル読み書きとコマンド実行までまかなう構成は[Appendix: Claude Code](appendix-claude-code.md)で扱います。
...
| 使用量アラートを設定せずに使い続ける | 月末の請求額で初めて問題に気づく | ツールとLLM双方に実行数・トークン数の上限を設定する |

最終行は、使い始めたあとに気づいても取り得る対応の幅が狭くなりがちです。アラート設定を最初に組み込んでおくと、原因の切り分けが進めやすくなります。
...
最初から完成形を狙わず、たたき台を人が校正する形で3週間ほど使ってから自動化の幅を広げる進め方が、使い方の作法までそろえるうえで扱いやすい順序になります。
