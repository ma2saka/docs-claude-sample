...
「毎朝、10個のアプリをだいたい同じ順番で開く」「週次で5枚のスクリーンショットをリサイズして1枚のスライドに貼る」——自分のPCで繰り返している手作業を、あらためて眺めてみると、意外と時間を食っていることに気づきます。本付録は、こうした**自分のデスクトップ（＝自分のPC）上の繰り返し作業**を、生成AIの力を借りてどこまで自動化できるのかを整理する地図です。
...
- **壊れやすい** — UIが少し変わっただけで、手順が丸ごと通らなくなる。人間なら「ボタンの色が変わったのね」と一瞬で気づく変化にも、エージェントは律儀に迷子になる
...
- 画面操作エージェントは目立つ働きができる反面、**遅い・高い・壊れやすい**の三重苦。APIが塞がっているときの選択肢として温存する
