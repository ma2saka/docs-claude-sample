...
2026年5月時点の特性は、実行速度の遅さ、トークン消費の多さ、UI変更による動作停止の起こりやすさの3点です。APIや連携機能が公開されていないアプリを画面越しに操作したい場面で選ぶ道具と位置づけると、用途が定まります。
...
なお、AnthropicのComputer Useは2026年4月のClaude Opus 4.7のGAで、扱える画面解像度の上限と、1回の依頼で割り当てられるターン数・計算リソースが引き上げられました。文字の小さなUIや複数手順をまたぐ操作で、取り違えが起きにくくなったとの観察が共有されています。2026年5月にはエージェント処理の判断がさらに改善した **Opus 4.8** がGAし、Opus系統の現行版にあたります（最終確認：2026-06-08）。
