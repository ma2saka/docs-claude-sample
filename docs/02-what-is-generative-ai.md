...
メモを長期保存する仕組みは別途用意されており、各社の「メモリ機能」や「プロジェクト知識」が当てはまります。「学習」という言葉との混同が起きやすい領域のため、[5章（「学習」というキーワードの誤解）](05-misunderstanding-learning.md)で、両者の区別と合わせて扱います。
...
呼び名はプロバイダ・モデルごとに異なります。Claudeでは `extended thinking`、Geminiでは `Deep Think`、OpenAI系では `o`系列の推論モデルというように、表記はそろっていません。これらはいずれも同じ系統の機能を指しており、社内資料や外部記事の比較もこの対応関係に立って書かれています。Gemini側の具体例としては、Gemini 3.1 Deep ThinkがGoogle AI Ultraサブスクで利用でき、現行世代の代表例として位置付けられます（最終確認：2026-05-16）。Claude側の具体例としては、2026年5月にGAされたClaude Opus 4.8がこれに該当し、`adaptive thinking` の方式が採用されています（最終確認：2026-06-11）。各社のラインアップでの扱いは[11章](11-gemini-advanced.md)・[13章](13-claude.md)で取り上げます。
