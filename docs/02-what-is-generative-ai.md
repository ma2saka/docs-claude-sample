...
| LLM（大規模言語モデル） | そのうち文章の生成を担う中核の計算モデル | GPT-5.5、Gemini 3.5、Claude Opus |
...
呼び名はプロバイダ・モデルごとに異なります。Claudeでは `extended thinking`、Geminiでは `Deep Think`、OpenAI系では `o`系列の推論モデルというように、表記はそろっていません。これらはいずれも同じ系統の機能を指しており、社内資料や外部記事の比較もこの対応関係に立って書かれています。Gemini側の具体例としては、2026年5月のGoogle I/Oで一般提供が始まった **Gemini 3.5** 系が現行世代にあたり、思考量を多めに取るDeep Thinkも選べます（最終確認：2026-06-08）。Claude側の具体例としては、2026年5月にGAされた **Claude Opus 4.8** が現行世代にあたり、`adaptive thinking` を引き継いでいます（最終確認：2026-06-08）。各社のモデル並びでの位置づけは[11章](11-gemini-advanced.md)・[13章](13-claude.md)で扱います。
