...
## 画像や音声は、専用の前段でベクトル表現に揃えてから後段の計算へ渡す

画像や音声を扱う生成AIは、テキスト以外の素材を、それぞれ専用の前段（画像エンコーダ、音声エンコーダなど）でモデルが扱えるベクトル表現に変換します。後段は、本章で見た次トークン予測の計算で応答を組み立てます。1章でGeminiに画像をアップロードして説明を求めたときも、内部では画像が前段でベクトル表現へ変換され、その表現をテキストの埋め込みと並べて扱える形に揃えたうえで、後段の確率計算が応答を組み立てていました。前段の処理は様式ごとに違いがあり、テキストのトークン化と同一ではありませんが、後段の計算はテキストと共通です。
