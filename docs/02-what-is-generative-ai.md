...
日常の利用範囲では、トークン数を直接数える場面はそれほどありません。一方で、課金の単位や、一度にモデルへ渡せる文章量の上限（コンテキストウィンドウ）の単位として後の章で再登場するため、用語だけ先に導入しておきます。詳細は[7章（用語集）](07-terminology.md)を参照してください。

## 「生成AI」「LLM」「チャット画面」は3層で整理する

[1章](01-gemini-in-workspace.md)で操作したのは、正確には「Geminiというチャット画面」です。生成AI、LLM、チャット画面の3語は混同されがちですが、指している階層が違います。
...
ここまでの内容を踏まえると、[1章](01-gemini-in-workspace.md)で観察された4つの挙動は、それぞれ中核の計算から説明できます。

### 毎回答えが微妙に違う

同じ質問を2回入力しても、まったく同じ文面は返ってきません。確率分布からサンプリングしている以上、毎回同じ単語列が選ばれるとは限らないためです。サイコロを2回振って同じ目が出るとは限らないのと同じ原理です。

プロバイダによっては「温度（temperature）」という設定で、このランダムさを調整できます。値を下げると回答は安定しますが、無難な文面に寄りやすくなります。チャット画面では利用者側から温度を変更できない場合が多く、応答にある程度の揺らぎが残る前提で使います。
...
メモを長期保存する仕組みは別途用意されており、各社の「メモリ機能」や「プロジェクト知識」がそれに当てはまります。「学習」という言葉との混同が起きやすい領域のため、[5章（「学習」というキーワードの誤解）](05-misunderstanding-learning.md)で、両者の区別と合わせて扱います。
...
- **② 指示追従の学習** — 望ましい応答のお手本（「こう聞かれたらこう答える」など）を人間が用意し、追加で覚えさせる
...
2024年以降、各社のモデルには、応答を返す前にいったん内部で長めに考えてから回答を組み立てるモードが用意されるようになりました。**推論モデル**（reasoning model）、あるいは思考モードと呼ばれます。同じ「次のトークンを確率で選ぶ」計算を基礎にしていますが、利用者から見える応答の前に、内部で考察やステップ分解に相当するトークン列を多く生成します。その分、回答までに時間と費用はかかりますが、難問での精度が上がりやすい、という観察が共有されています。
...
呼び名はプロバイダ・モデルごとに異なります。Claudeでは `extended thinking`、Geminiでは `Deep Think`、OpenAI系では `o`系列の推論モデルというように、表記はそろっていません。これらはいずれも同じ系統の機能を指しており、社内資料や外部記事の比較もこの対応関係に立って書かれています。Gemini側の具体例としては、Gemini 3.1 Deep ThinkがGoogle AI Ultraサブスクで利用でき、現行世代の代表例です（最終確認：2026-05-16）。Claude側の具体例としては、2026年5月にGAされたClaude Opus 4.8がこれに該当し、`adaptive thinking` の方式が採用されています（最終確認：2026-06-11）。各社のラインアップでの扱いは[11章](11-gemini-advanced.md)・[13章](13-claude.md)で取り上げます。
...
拡散型は、文章をノイズ（でたらめな単語列）で埋めた状態から始め、全体を数ステップかけて一斉に洗練させて完成形に近づけます。1語ずつ書き進める書き手に対して、下書き全体をまとめて推敲する編集者に近い方式です（書き手→自己回帰型、編集者→拡散型）。前のトークンを待たずにブロック単位で並列処理できるため、生成速度を上げやすい性質を持ちます。
...
- Inception LabsのMercuryシリーズ — 商用の拡散型モデルとしてAPI経由で利用できる
...
利用者がチャット画面の裏側の生成方式を意識する場面は多くありません。本節で押さえたいのは、精度を優先するモデルと応答速度を優先するモデルの間の選択肢が、方式の面からも広がりつつあるという流れです。モデルの段構成での見方は[8章](08-common-capabilities.md)で扱います。
...
苦手側に並ぶ作業を任せた場合は、もっともらしい嘘の混入が増えます。得意側に並ぶ作業であれば、下書きとして使える応答が返りやすくなります。
...
画像や音声を扱う生成AIも、処理の大きな流れは文章のときと近い形に整理できます。画像はピクセル、音声は音の波形を、それぞれモデルが処理できるベクトル表現に変換する前段が入り、表現がそろった後の生成は文章のときと近い枠組みで動きます。前段の実装はモデルごとに異なり、専用のエンコーダ層やembedding空間への射影など、テキストとは別の処理が組み合わさります。[1章](01-gemini-in-workspace.md)でGeminiに画像をアップロードして説明を求めたときも、内部ではいったん画像をモデルが扱える表現に変換したうえで応答が生成されていました。文章とは別の仕組みではなく、同じモデルが扱える表現に前段でそろえている、と整理できます。

入出力の組み合わせ方や、業務で扱う際の要点は、独立した章にまとめてあります。詳しくは[3章（マルチモーダル）](03-multimodal.md)を参照してください。
