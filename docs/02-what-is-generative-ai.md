...
2024年以降、各社のモデルには、応答を返す前にいったん内部で長めに考えてから回答を組み立てるモードが用意されるようになりました。**推論モデル**、あるいは**思考モード**と呼ばれます。同じ「次のトークンを確率で選ぶ」計算の上に乗っていますが、利用者から見える応答の前に、内部で考察やステップ分解にあたるトークン列を多く生成する分、回答までに時間と費用がかかる代わりに、難問での精度が上がりやすいという観察が共有されています。
...
呼び名はプロバイダ・モデルごとに揺れます。Claudeでは `extended thinking`、Geminiでは `Deep Think`、OpenAI系では `o`系列の推論モデルというように、表記がそろっていません。社内資料や外部記事を読むときは、これらが同じ系統の機能を指していると押さえておくと、対比がしやすくなります。各社のモデル並びでの位置づけは[11章](11-gemini-advanced.md)・[13章](13-claude.md)で扱います。

業務での使い分けの目安としては、定型作業や下書きには通常モード、設計の整合性確認や数値・条件の絡む検証には推論モードを選ぶ、という分け方が手がかりになります。費用と所要時間が増える分、すべての依頼を推論モードで通す必要はありません。
