...
この確率計算は、**Transformer**と呼ばれるニューラルネットワークの構造の上で実装されています。中核に置かれているのが**アテンション**（self-attention、自己注意）と呼ばれる仕組みです。アテンションは、入力に含まれる各トークンが、文脈中の他のどの位置をどれくらい参照するかを動的に重みづけし、次のトークンの確率分布の計算へつなげます。本ドキュメントでは数式や層構成までは踏み込みません。記事や社内資料で「Transformerベース」「アテンション機構」という語に出会ったとき、本節で扱った次トークン予測の計算を支える土台を指している、と接続できれば十分です。Transformer以外のアーキテクチャも研究されていますが、現行の主要なLLMはおおむねTransformerを基盤にしています。用語の再確認は[7章（用語集）](07-terminology.md)に揃えてあります。
