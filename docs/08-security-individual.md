...
本章の前提はシンプルです。読者は組織のガバナンスを設計する立場ではなく、ただ仕事を前に進めたい一人のユーザーです。「気をつけるべきことが多すぎて、結局何もできない」という状態にならないよう、論点を3つの箱に分けて、順に手渡していきます。組織側がなぜルールを敷くのかという背景は、続く[9章](09-security-agent-era.md)の冒頭で扱います。
...
- 組織側がなぜルールを敷くのか、その理屈と、エージェント時代に増える論点は [9章（セキュリティ エージェント時代のガバナンス）](09-security-agent-era.md) で扱う
