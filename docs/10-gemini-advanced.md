...
[1章](1-gemini-in-workspace.md)でGeminiの入口を体験し、[7章](7-common-capabilities.md)でチャット・アーティファクト・コネクタという共通の骨格を学びました。本章はその続きで、「Gemini固有の手ざわり」だけを取り上げます。Claudeと並べると際立つGeminiの個性、知っておきたい現時点の制約、使い分けの感覚、の3点を軸に進めます。
...
- [1章](1-gemini-in-workspace.md)でGeminiに触れ、[7章](7-common-capabilities.md)を一読した人
...
- 役割を1つに絞る。「なんでもできるGem」は、何も得意でないGemになりがち
- 口調の指定を忘れずに。読者層（社内向け/社外向け）と文体（です・ます/箇条書き中心など）を書いておくと出力がそろう
- ナレッジファイルは小さく保つ。詰め込みすぎるとコンテキストウィンドウを圧迫し、精度低下につながる場合もある

プランによっては、作ったGemをチーム内で共有できます。共有の可否や手順は、Googleの公式ドキュメントで確認してください（最終確認：2026-04-24）。

## Canvas: 成果物を仕上げる作業台

[7章](7-common-capabilities.md)でアーティファクトの概念を扱いました。GeminiにおけるアーティファクトがCanvasです。チャット画面の横に作業ペインが開き、文書やコードを「対話しながら磨く」ことができます。
...
ただし、共有リンクは設定次第で社外からも閲覧できます。社外秘の内容を含む場合は公開範囲の設定を必ず確認してから共有してください（詳細は[8章](8-security-individual.md)）。
...
複数の素材を組み合わせることもできます。「この契約書PDFとこの会議メモ画像をもとに、論点を整理して」という依頼が1回で出せます。素材が多いほど文脈が豊かになる反面、コンテキストウィンドウの上限（[6章](6-terminology.md)）に近づくのも早くなります。全部まとめて投げる前に、まず要点だけ渡して様子を見る、という進め方が安定します。
...
Geminiを積極的に使い始めると、「あれ、できないの？」と感じる場面がいくつかあります。代表的な制約を先に知っておくと、余計な試行錯誤が減ります。

### メール送信はまだ手動

GmailのサイドパネルからGeminiに返信下書きを作らせることはできますが、**送信ボタンを押すのは人間の仕事**です。現時点では、Geminiがユーザーの意図を読んで自動的にメールを送り出す経路は標準では用意されていません。

「あとは任せた」と退席するのではなく、「下書き → 確認 → 送信」の3段階をそのまま踏んだほうが、現時点では現実的です。

### 外部のMCPには直接繋がらない

[3章](3-external-system-integration.md)で紹介したModel Context Protocol（MCP）は、Claude側では積極的に採用されています。一方Geminiは、2026年4月時点では、任意のMCPサーバを自前でセットアップして自由に繋ぐ、という形の利用は標準的なチャット画面からは難しい状況です。Google製のコネクタ（Gmail・Drive・カレンダーなど）は豊富ですが、「社内独自ツールをMCPサーバとして立ててGeminiに繋ぐ」のような使い方はClaudeのほうが柔軟です。
...
## GeminiとClaude、使い分けの感覚

7章で「Workspaceの内側はGeminiが近道」と書きましたが、もう少し粒度を上げて整理します。

| 状況 | 向いているほう | 理由 |
| ---- | ---- | ---- |
| GmailやDocsの画面から離れずに手伝ってほしい | Gemini | サイドパネル統合が距離ゼロ |
| 画像・音声・動画を混ぜた素材処理 | Gemini | マルチモーダルの対応幅が広い |
| 社内の特定ツールにMCPで繋いで自動化したい | Claude | MCP対応の自由度が高い |
| コードの推敲や複雑な対話を深く続けたい | Claude | 長い文脈での一貫性が安定しやすい |
| プリセット（Gems）で反復作業を効率化したい | Gemini | Gemsの設計がシンプル |
| 成果物をGoogleドキュメントに流し込みたい | Gemini | CanvasからDocsへのエクスポートがそのまま使える |

この表は2026年4月時点の傾向であり、数か月後には変わっている可能性があります。「とりあえず使い始めてみて、壁にぶつかったら乗り換える」くらいの軽さで構えてください。道具の適材適所は使い続けるうちに体でわかっていくものです。
