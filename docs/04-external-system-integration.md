...
本章は、生成AIが外部システムと繋がる仕組みを、「**ツール呼び出し**」という1つの仕組みに集約して整理します。チャット欄しか持たないAIがGmailを読んだりカレンダーを参照したり社内文書を引いたりできるのは、AI本体がネットワークに出ているからではなく、外側のプログラムが依頼文を受け取って代わりに実行する経路があるからです。以降の章で登場するコネクタ、MCP、エージェントといった用語は、いずれもこのツール呼び出しを前提に構成されます。
...
語をより広い文脈で押さえたくなった場合は、[7章](07-terminology.md)の該当項目を参照すれば、そのまま本章に戻れます。
...
ここで確認しておきたいのは、AI本体はネットワークに出ていない点です。AIは依頼文を出力するだけで、実際にネットワークへ出ていくのは外側のプログラムです。客がレストランで厨房に直接行かず、店員にオーダー票を渡す関係に対応します。客がユーザー、店員がAI、オーダー票がツール呼び出しの依頼文、厨房が外部ツールにそれぞれ対応します。
...
続いて、ツール使用の有無を一枚にまとめた抽象フローを示します。従来のチャットとの違いは、途中にツール呼び出しのループが入るかどうかの一点に集約されます。
...
MCPは、ツール呼び出しを共通プロトコルとして標準化した規格です。Anthropicが2024年に公開し、現在は他社クライアントの対応も進んでいます。役割としてはUSB-Cに近く、AIごとに異なる形のプラグを個別に自作していたところを、規格化によってひとつのMCPサーバを立てれば対応クライアントから同じ作法で呼べる形に変わりました。対応関係としては、MCPプロトコルがUSB-Cという規格の側に立ち、各MCPサーバはそこに接続される周辺機器の側を担います。

MCPの具体的な使いどころは、[Appendix: Claude Code](appendix-claude-code.md)で扱います。本章では、ツール呼び出しに標準規格が生まれたという事実を確認できれば、先の章を読み進める前提が整います。

## ツール呼び出しに関するよくある誤解

ツール呼び出しは、「AIが自ら動いて操作している」という見方と結びつきやすく、誤解も生まれやすい領域です。代表的なものを先に並べます。

| 誤解 | 実際はどうか |
| ---- | ---- |
| AIが勝手にインターネットに飛び出していく | 外側のプログラムが、事前に許可された道具しか実行しない。AIは依頼文を出力するだけ |
| 「ツール」は画面を自動操作するものを指す | 多くはAPI経由のデータ取得。画面操作型は別ジャンル（[Appendix: デスクトップの自動化](appendix-desktop-automation.md)参照） |
...
## A2A Protocolはエージェント間の連携を標準化する隣接規格

ツール呼び出しを共通化するMCPに対して、エージェントどうしの連携を標準化する規格も整いつつあります。代表例が **A2A（Agent2Agent）Protocol** です。MCPが「エージェントから外部ツールを呼ぶ」ための標準であるのに対し、A2Aは「異なる基盤で動くエージェントどうしがやり取りする」ための標準で、両者は層が異なります。2026年春時点で、A2AはLinux Foundationの配下に置かれたAgentic AI Foundationのプロジェクトとしてv1.2に到達し、150を超える組織が本番運用に組み込んでいると公表されています。エージェントが自分の身元として提示する情報（agent card）は暗号署名で検証されるため、組織横断でやり取りする場合でも相手を確認したうえで通信を進められます。本ドキュメントの想定読者がA2Aを直接実装する場面は前提に置いていませんが、社内資料や外部記事で名前を見かけたときに、MCPに隣接する層として位置づけられれば十分です。

なお、Google Cloud Next 2026では、Vertex AIの発展形としてGemini Enterprise Agent Platformが発表されました。Google Cloudのエージェント基盤を担う製品で、エンジニア向けの題材のため、本ドキュメントでは深追いしません。
...
コネクタはツールそのもの、MCPはツールを呼び出す共通規格、エージェントはツール呼び出しを連ねて目的の成果物にたどり着く仕組み、と分けておくと混同しにくくなります。いずれも基盤にはツール呼び出しがあります。後続の章でこれらの概念を扱う際は、本章の仕組みに立ち戻って整理できます。とくにコネクタの業務での使いどころは、[8章（生成AIでできること 共通編）](08-common-capabilities.md)でチャット・アーティファクトと並ぶ3本柱の1つとして整理しています。Gemini／Claudeそれぞれの製品ごとの使い方は、[11章（Geminiを使いこなそう）](11-gemini-advanced.md)・[13章（Claudeを使いこなそう）](13-claude.md)で扱います。
...
次は[5章（「学習」というキーワードの誤解）](05-misunderstanding-learning.md)で、ツール呼び出しと混同されやすい「学習」という言葉の整理に進みます。コネクタやMCPの仕組みをZapier・Make・n8nなどのSaaS連携ツールに組み込む実践は、[Appendix: ワークフローツール](appendix-workflow-tools.md)で扱います。
...
- A2A Project「A2A Protocol」: <https://a2a-protocol.org/latest/>（最終確認：2026-05-16）
- Linux Foundation Press Release「A2A Protocol Surpasses 150 Organizations」: <https://www.linuxfoundation.org/press/a2a-protocol-surpasses-150-organizations-lands-in-major-cloud-platforms-and-sees-enterprise-production-use-in-first-year>（最終確認：2026-05-16）
- Google Cloud Blog「Introducing Gemini Enterprise Agent Platform」: <https://cloud.google.com/blog/products/ai-machine-learning/introducing-gemini-enterprise-agent-platform>（最終確認：2026-05-16）
