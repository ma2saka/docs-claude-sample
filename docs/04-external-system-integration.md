...
MCPは、ツール呼び出しを共通プロトコルとして標準化した規格です。Anthropicが2024年に公開し、現在は他社クライアントの対応も進んでいます。役割としてはUSB-Cに近く、AIごとに異なる形のプラグを個別に自作していたところを、規格化によってひとつのMCPサーバを立てれば対応クライアントから同じ作法で呼べる形に変わりました。対応関係としては、MCPプロトコルがUSB-Cという規格の側にあたり、各MCPサーバはそこに接続される周辺機器の側にあたります。
...
ツール呼び出しを共通化するMCPに対して、エージェントどうしの連携を標準化する規格も整いつつあります。代表例が **A2A（Agent2Agent）Protocol** です。MCPは「エージェントから外部ツールを呼ぶ」標準で、A2Aは「異なる基盤で動くエージェントどうしがやり取りする」標準にあたり、両者は層が異なります。2026年春時点で、A2AはLinux Foundationの配下に置かれたAgentic AI Foundationのプロジェクトとしてv1.2に到達し、150を超える組織が本番運用に組み込んでいると公表されています。エージェントが自分の身元として提示する情報（agent card）は暗号署名で検証されるため、組織横断でやり取りする場合でも相手を確認したうえで通信を進められます。本ドキュメントの想定読者がA2Aを直接実装する場面は前提に置いていませんが、社内資料や外部記事で名前を見かけたときに、MCPに隣接する層として位置づけられれば十分です。

なお、Google Cloud Next 2026では、Vertex AIの発展形としてGemini Enterprise Agent Platformが発表されました。Google Cloudのエージェント基盤にあたり、エンジニア向けの題材のため、本ドキュメントでは深追いしません。
