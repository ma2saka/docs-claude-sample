...
MCPの具体的な使いどころは、[Appendix: Claude Code](appendix-claude-code.md)で扱います。本章ではツール呼び出しに標準規格が生まれた事実を押さえれば、先の章を読み進める前提として十分です。
...
コネクタはツールそのもの、MCPはツールを呼び出す共通規格、エージェントはツール呼び出しを連ねて目的の成果物にたどり着く仕組み、と分けておくと混同しにくくなります。いずれも基盤にはツール呼び出しがあります。後続の章でこれらの概念を扱う際は、本章の仕組みに立ち戻って整理できます。とくにコネクタの業務での使いどころは、[8章（生成AIでできること 共通編）](08-common-capabilities.md)でチャット・アーティファクトと並ぶ3本柱の1つとして整理しています。ブランド／製品ごとに利用者から見える形は、Gemini側は[11章（Geminiを使いこなそう）](11-gemini-advanced.md)で、Claude側は[13章（Claudeを使いこなそう）](13-claude.md)で扱います。
