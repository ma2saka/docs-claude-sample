...
カットオフ以後の情報は、会話のコンテキストに入れれば参照できます。添付資料、Webコネクタの取得結果、RAG（検索拡張）で引いた文書などが、その経路の例です。コンテキストに入れた情報は、モデルの重みを書き換えないまま、一時的な参照対象として扱われます。RAGや埋め込み検索の仕組みは[Appendix: 拡張検索と埋め込み検索](appendix-retrieval-and-embeddings.md)で扱います。
