...
[2章](02-what-is-generative-ai.md)で触れたとおり、大規模言語モデルの動作の中心は「次に来るトークンを確率で選ぶ」仕組みです。事前学習の段階では、訓練の中心は自然な続きを選ぶことであり、生成された文の真偽そのものを直接の評価軸にはしていません。事後学習で事実性を高める調整が重ねられるとはいえ、出力ごとに真偽を機械的に検証する仕組みが組み込まれているわけではありません。

モデルから見れば、空白を埋めないことは応答として不自然に扱われやすい一方、埋め方の真偽が出力時に直接照合されるわけではありません。ここが第一のずれです。

### 棄権の訓練不足

近年のモデルは、事後のチューニングで「分からないときは分からないと言う」振る舞いをかなり学んでいます。とはいえ、事実とごく近い誤りや、専門性の高い話題では、棄権より「それらしく答える」を選びがちです。

棄権は、明示的に許可されない限り選ばれにくい振る舞いです。学習データのうち、「分かりません」で終える文章の比率は限られているため、確率的にも「何かを答える」側に偏ります。利用者の側で「分からないときは分からないと書いてよい」と明示すると、棄権の選択肢を取りやすくなります。
...
注意が必要なのは、コンテキストで渡した情報が常に優先される、とは限らない点です。モデルは会話履歴と学習済みの常識の双方を参照しており、矛盾があるときに必ず「与えられた資料を正」と判断するわけではありません。プロンプトで「この資料を優先してください」と明示するのが安全です。
