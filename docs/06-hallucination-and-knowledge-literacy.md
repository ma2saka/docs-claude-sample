...
AIに何かを尋ねると、ごく自然な日本語で、しかも言い切りの強い調子で間違いを返してくることがあります。これが「ハルシネーション」と呼ばれる現象です。文面に自信があるからといって、中身の正しさが保証されているわけではない、というのが注意しておきたい点です。
...
- 5章で「学習」の分類を踏まえた人（事前学習・ファインチューニング・コンテキスト／メモリ）
- 用語で詰まったら、7章（モデル、コンテキスト、トークンなど）に戻ってください

本章の結論を先に一行で言うと、「**AIは知らないことを知らないと言うのが苦手**」ということです。苦手な理由を理解し、苦手をカバーする段取りをこちらで作る、というのが本章の着地点です。
...
どれも「悪意をもって騙そうとしている」わけではありません。次の節で見るように、**そう答えるのが自然だと判断した結果**なのです。動機のないまま結果として嘘が混じる、という形ゆえに扱いが難しくなります。
...
棄権の訓練は、学校のテストで言えば「白紙答案もOKだよ」と学ぶ練習に似ています。日本の受験文化と同じで、AIも白紙の回答を選びにくい傾向がある、と捉えておくと実務感覚と合わせやすくなります。

### 文脈の揺らぎ

長い会話になればなるほど、会話の最初に与えた前提や制約は、文脈の端っこに押しやられます。コンテキストウィンドウからはみ出した情報は、7章で触れたとおり**単純に無視**されます。

この「揺らぎ」は静かに進みます。最初の回答は正しかったのに、10往復目で急に別人のことを話し始めた、という経験がある人もいるはずです。モデルが嘘をついたのではなく、最初に渡した前提が会話履歴の後ろへ押しやられ、モデルの目に入らなくなっている状態、と捉えてください。

### 曖昧な依頼

「いい感じに書いて」「詳しく教えて」という依頼は、モデルに広い選択肢を与えます。選択肢が広いほど、埋め方のブレ幅は大きくなり、事実にあたる部分も一緒に揺れます。

「問いを詳しく書くとハルシネーションが減る」のは根性論ではなく、こうした確率的な性質の裏返しです。
...
「じゃあ最新モデルに乗り換えれば最新情報が分かるのか」と思うかもしれませんが、そう単純でもありません。カットオフが近い日付でも、**その直前の出来事は学習データ中で十分に書かれていないことが多く、結果としてその時期の情報は弱くなる**傾向があります。新しい話題ほど信頼度が下がりやすい、という前提を踏まえておくと、事故が起きやすい局面を見極めやすくなります。
...
ここからは、受け取った出力を前にしたときの実務の話です。結論から言えば、**裏取りの責任はAIではなく使う側にある**、ということに尽きます。AIの出力は、言い切りの強い新人の下書きだと思って受け取り、最終的な責任は使う側が持つ、という前提で扱うのが無難です。

### 裏取りが必須の領域／甘くてよい領域

すべての出力に同じ熱量で裏を取る必要はありません。リスクの軽重で4段階ほどに分けると、現実的に回せます。
...
下の段ほど「嘘が混じっても実害が小さい」領域だと考えてください。

### 裏取りの具体的なやり方

実務で回しやすいやり方を、手間の軽いものから重いものへ順に並べました。

- **自己照合** — モデル自身に「その情報源のURLを教えてください」と追加で聞く。出てくれば踏む、踏めなければ要警戒
- **別モデルで再質問** — Claudeに聞いた事実をGeminiで確認する、あるいは逆。答えがそろわなければ、少なくとも一方、ときに両方が怪しい
- **一次ソース当たり** — 公式ドキュメント、論文、法令、プレスリリースに直接当たる。手間はかかるが、対外資料なら必須
- **検索結果との突き合わせ** — Web検索機能つきのモードで尋ね、引用URLをそのまま踏む

自己照合はハルシネーション対策としては限定的な効果しか期待できません。モデルは、存在しないURLや存在しない論文タイトルも、もっともらしく返します。必ずリンクを踏み、到達ページの中身まで目で確認してください。
...
- **範囲の限定**。たとえば「学習カットオフ以降の価格や仕様については推測せず、その旨を書いてください」のように、対象の期間や領域を明示する

棄権の許容は、手軽なわりに違いが出やすい指示です。モデルは、明示的に許可されると、「分かりません」と答える選択肢を取りやすくなります。
...
3番目の「揺らぎテスト」は、手間のわりに見分け手段として役立ちます。同じ事実を別の文脈で聞いたときに答えが一貫しない情報は、モデル自身も確信を持っていないサインです。
...
- 依頼のときに「根拠を示す／分からないなら分からないと言う」を明示するだけでも、事故に至る局面を減らせる
