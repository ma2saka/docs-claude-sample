...
[2章](02-what-is-generative-ai.md)で触れたとおり、大規模言語モデルの中核は「次に来るトークンを確率で選ぶ」仕組みです。訓練の目標は「事実を正しく答えること」ではなく、「人間が書きそうな続きを選ぶこと」です。両者は一致する場面が多いだけで、設計上は別の目的になります。

モデルから見れば、空白を埋めないことはコスト（不自然な文）として扱われる一方、埋め方の真偽はコストとして直接測られません。ここが第一のずれです。
