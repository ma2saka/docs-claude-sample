...
本編は1〜12章の12章構成で、本章（0章）はその地図役、末尾に現場向けの付録が3本付きます。章はおおまかに4つのかたまりに分かれます。

```mermaid
flowchart TB
    A["0・1章: 導入と体験"] --> B["2〜6章: 土台の理解"]
    B --> C["7章: 共通編（できること）"]
    C --> F["8・9章: セキュリティ（個人／エージェント時代）"]
    F --> D["10・11章: Gemini側の使いこなし"]
    F --> E["12章: Claude側の使いこなし"]
```
