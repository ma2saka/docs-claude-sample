...
個別の章を設けるのはClaudeとGeminiの2サービスです。Google Workspaceを日常の業務環境とする読者にとって、Geminiは追加の登録なしで使い始められる位置にあり、ClaudeはWorkspaceの外側の作業を補う選択肢にあたります。ChatGPTやMicrosoft Copilotには個別章を設けませんが、2〜10章で整理する概念（プロンプト、ハルシネーション、コンテキスト、セキュリティなど）はサービスを問わず共通です。ChatGPTやCopilotを普段使いしている読者にも、この範囲の内容はそのまま当てはまります。
