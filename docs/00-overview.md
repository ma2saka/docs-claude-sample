...
## 本編13章は4ブロック構成、目的別の最短ルートも用意した

本編は1〜13章の構成で、本章（0章）はその地図にあたります。末尾には現場向けの付録が3本付きます。本編の章は、おおまかに次の4つのブロックに分かれます。
...
付録3本は、本編から少し離れた領域を扱います。日常業務寄りの順に「ワークフローツール」（Zapierやn8nによるSaaS同士の連携）、「デスクトップの自動化」（自席PCの繰り返し作業）、「Claude Code」（開発者寄りの入口）と並びます。本編を読み終えたあと、必要になったタイミングで参照する想定です。
